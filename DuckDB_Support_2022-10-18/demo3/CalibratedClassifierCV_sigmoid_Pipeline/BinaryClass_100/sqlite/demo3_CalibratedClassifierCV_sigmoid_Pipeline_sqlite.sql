...
-- Code For temporary table TMP_20221018184917_KBHB1OU_ADS_imp_1_OUT part 1. Create 


CREATE TEMPORARY TABLE "TMP_20221018184917_KBHB1OU_ADS_imp_1_OUT" (
	"KEY" BIGINT NOT NULL, 
	impter_2 FLOAT, 
	impter_3 FLOAT, 
	impter_4 FLOAT, 
...
-- Code For temporary table TMP_20221018184917_KBHB1OU_ADS_imp_1_OUT part 3. Create Index 

CREATE INDEX "ix_TMP_20221018184917_KBHB1OU_ADS_imp_1_OUT_KEY" ON "TMP_20221018184917_KBHB1OU_ADS_imp_1_OUT" ("KEY")

-- Code For temporary table TMP_20221018184917_SF35O99_ADS_sca_2_OUT part 1. Create 


CREATE TEMPORARY TABLE "TMP_20221018184917_SF35O99_ADS_sca_2_OUT" (
	"KEY" BIGINT NOT NULL, 
	scaler_2 FLOAT, 
	scaler_3 FLOAT, 
	scaler_4 FLOAT, 
...
FROM "TMP_20221018184917_KBHB1OU_ADS_imp_1_OUT" AS "ADS_imp_1_OUT") AS "ADS_sca_2_OUT") AS "T"

-- Code For temporary table TMP_20221018184917_SF35O99_ADS_sca_2_OUT part 3. Create Index 

CREATE INDEX "ix_TMP_20221018184917_SF35O99_ADS_sca_2_OUT_KEY" ON "TMP_20221018184917_SF35O99_ADS_sca_2_OUT" ("KEY")

-- Code For temporary table TMP_20221018184917_QP5E20B_CalProb_B0 part 1. Create 


CREATE TEMPORARY TABLE "TMP_20221018184917_QP5E20B_CalProb_B0" (
	"KEY" BIGINT, 
	"Proba_1" FLOAT, 
	"Proba_0" FLOAT
)
...
(SELECT "Calibrated_Model_0"."KEY" AS "KEY", 1.0 / (1.0 + exp(min(100.0, max(-100.0, -(2.1972249883101176 * "Calibrated_Model_0"."ScoreOrProba_1" + -1.098613312520129))))) AS "Proba_1", 1.0 - 1.0 / (1.0 + exp(min(100.0, max(-100.0, -(2.1972249883101176 * "Calibrated_Model_0"."ScoreOrProba_1" + -1.098613312520129))))) AS "Proba_0" 
...
"DT_node_lookup_1" AS 
(SELECT "ADS_sca_2_OUT"."KEY" AS "KEY", CASE WHEN ("ADS_sca_2_OUT".scaler_59 <= 0.06653057038784027) THEN CASE WHEN ("ADS_sca_2_OUT".scaler_73 <= -1.9187528491020203) THEN 2 ELSE CASE WHEN ("ADS_sca_2_OUT".scaler_11 <= 2.1140206456184387) THEN CASE WHEN ("ADS_sca_2_OUT".scaler_65 <= 2.1003255248069763) THEN CASE WHEN ("ADS_sca_2_OUT".scaler_96 <= 1.3444217443466187) THEN 6 ELSE 7 END ELSE 8 END ELSE 9 END END ELSE CASE WHEN ("ADS_sca_2_OUT".scaler_61 <= 1.090500384569168) THEN 11 ELSE 12 END END AS node_id_2 
FROM "TMP_20221018184917_SF35O99_ADS_sca_2_OUT" AS "ADS_sca_2_OUT"), 
"DT_node_data_1" AS 
(SELECT "Values".nid AS nid, "Values"."P_0" AS "P_0", "Values"."P_1" AS "P_1", "Values"."D" AS "D", "Values"."DP" AS "DP" 
...
(SELECT "Calibrated_Model_1"."KEY" AS "KEY", 1.0 / (1.0 + exp(min(100.0, max(-100.0, -(0.3375229379078027 * "Calibrated_Model_1"."ScoreOrProba_1" + -0.15180579266556016))))) AS "Proba_1", 1.0 - 1.0 / (1.0 + exp(min(100.0, max(-100.0, -(0.3375229379078027 * "Calibrated_Model_1"."ScoreOrProba_1" + -0.15180579266556016))))) AS "Proba_0" 
...
(SELECT "Calibrated_Model_2"."KEY" AS "KEY", 1.0 / (1.0 + exp(min(100.0, max(-100.0, -(1.7722622792314653 * "Calibrated_Model_2"."ScoreOrProba_1" + -0.9199647363838578))))) AS "Proba_1", 1.0 - 1.0 / (1.0 + exp(min(100.0, max(-100.0, -(1.7722622792314653 * "Calibrated_Model_2"."ScoreOrProba_1" + -0.9199647363838578))))) AS "Proba_0" 
...
(SELECT "Calibrated_Model_3"."KEY" AS "KEY", 1.0 / (1.0 + exp(min(100.0, max(-100.0, -(2.353584621839785 * "Calibrated_Model_3"."ScoreOrProba_1" + -1.5899821845615933))))) AS "Proba_1", 1.0 - 1.0 / (1.0 + exp(min(100.0, max(-100.0, -(2.353584621839785 * "Calibrated_Model_3"."ScoreOrProba_1" + -1.5899821845615933))))) AS "Proba_0" 
...
FROM sigmoid_calibration_3)
 INSERT INTO "TMP_20221018184917_QP5E20B_CalProb_B0" ("KEY", "Proba_1", "Proba_0") SELECT "T"."KEY", "T"."Proba_1", "T"."Proba_0" 
...
-- Code For temporary table TMP_20221018184917_OT67G61_CalProb_avg part 1. Create 


CREATE TEMPORARY TABLE "TMP_20221018184917_OT67G61_CalProb_avg" (
	"KEY" BIGINT NOT NULL, 
	"Proba_1" FLOAT, 
	"Proba_0" FLOAT, 
	PRIMARY KEY ("KEY")
...
-- Code For temporary table TMP_20221018184917_OT67G61_CalProb_avg part 2. Populate
...
FROM (SELECT "CalProb_B0"."KEY" AS "KEY", "CalProb_B0"."Proba_1" AS "Proba_1", "CalProb_B0"."Proba_0" AS "Proba_0" 
FROM "TMP_20221018184917_QP5E20B_CalProb_B0" AS "CalProb_B0") AS "CalProb_EnsembleUnion")
 INSERT INTO "TMP_20221018184917_OT67G61_CalProb_avg" ("KEY", "Proba_1", "Proba_0") SELECT "T"."KEY", "T"."Proba_1", "T"."Proba_0" 
...
-- Code For temporary table TMP_20221018184917_OT67G61_CalProb_avg part 3. Create Index 

CREATE INDEX "ix_TMP_20221018184917_OT67G61_CalProb_avg_KEY" ON "TMP_20221018184917_OT67G61_CalProb_avg" ("KEY")
...
FROM "TMP_20221018184917_OT67G61_CalProb_avg" AS "CalProb_avg"), 
