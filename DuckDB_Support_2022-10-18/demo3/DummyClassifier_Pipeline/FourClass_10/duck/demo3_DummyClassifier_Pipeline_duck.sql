...
-- Code For temporary table TMP_20221018185153_W5ICN0Y_ADS_imp_1_OUT part 1. Create 


CREATE TEMPORARY TABLE "TMP_20221018185153_W5ICN0Y_ADS_imp_1_OUT" (
	"KEY" BIGINT NOT NULL, 
...
-- Code For temporary table TMP_20221018185153_W5ICN0Y_ADS_imp_1_OUT part 3. Create Index 

CREATE INDEX "ix_TMP_20221018185153_W5ICN0Y_ADS_imp_1_OUT_KEY" ON "TMP_20221018185153_W5ICN0Y_ADS_imp_1_OUT" ("KEY")

-- Code For temporary table TMP_20221018185153_QJKRVBT_ADS_sca_2_OUT part 1. Create 


CREATE TEMPORARY TABLE "TMP_20221018185153_QJKRVBT_ADS_sca_2_OUT" (
	"KEY" BIGINT NOT NULL, 
...
-- Code For temporary table TMP_20221018185153_QJKRVBT_ADS_sca_2_OUT part 2. Populate

INSERT INTO "TMP_20221018185153_QJKRVBT_ADS_sca_2_OUT" ("KEY", scaler_output_2, scaler_output_3, scaler_output_4, scaler_output_5, scaler_output_6, scaler_output_7, scaler_output_8, scaler_output_9, scaler_output_10, scaler_output_11) SELECT "T"."KEY", "T".scaler_output_2, "T".scaler_output_3, "T".scaler_output_4, "T".scaler_output_5, "T".scaler_output_6, "T".scaler_output_7, "T".scaler_output_8, "T".scaler_output_9, "T".scaler_output_10, "T".scaler_output_11 
...
FROM "TMP_20221018185153_W5ICN0Y_ADS_imp_1_OUT" AS "ADS_imp_1_OUT") AS "ADS_sca_2_OUT") AS "T"

-- Code For temporary table TMP_20221018185153_QJKRVBT_ADS_sca_2_OUT part 3. Create Index 

CREATE INDEX "ix_TMP_20221018185153_QJKRVBT_ADS_sca_2_OUT_KEY" ON "TMP_20221018185153_QJKRVBT_ADS_sca_2_OUT" ("KEY")

-- Model deployment code

WITH dummy_class_reg_cte AS 
(SELECT "ADS_sca_2_OUT"."KEY" AS "KEY", 0.225 AS "Proba_0", CASE WHEN (0.225 IS NULL OR 0.225 > 0.0) THEN ln(0.225) ELSE '-Infinity'::float8 END AS "LogProba_0", 0.25 AS "Proba_1", CASE WHEN (0.25 IS NULL OR 0.25 > 0.0) THEN ln(0.25) ELSE '-Infinity'::float8 END AS "LogProba_1", 0.25 AS "Proba_2", CASE WHEN (0.25 IS NULL OR 0.25 > 0.0) THEN ln(0.25) ELSE '-Infinity'::float8 END AS "LogProba_2", 0.275 AS "Proba_3", CASE WHEN (0.275 IS NULL OR 0.275 > 0.0) THEN ln(0.275) ELSE '-Infinity'::float8 END AS "LogProba_3", 3 AS "Decision", CASE WHEN (3 = 0) THEN 0.225 WHEN (3 = 1) THEN 0.25 WHEN (3 = 2) THEN 0.25 WHEN (3 = 3) THEN 0.275 END AS "DecisionProba" 
FROM "TMP_20221018185153_QJKRVBT_ADS_sca_2_OUT" AS "ADS_sca_2_OUT")
