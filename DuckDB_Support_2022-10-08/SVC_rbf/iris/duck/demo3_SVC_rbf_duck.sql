...
-- Code For temporary table TMP_20221008175354_L6G14LW_KernAgg_B0 part 1. Create 


CREATE TEMPORARY TABLE "TMP_20221008175354_L6G14LW_KernAgg_B0" (
	"KEY" BIGINT, 
	"OVO_Class1" BIGINT, 
	"OVO_Class2" BIGINT, 
	"OVO_Decision" BIGINT, 
...
-- Code For temporary table TMP_20221008175354_L6G14LW_KernAgg_B0 part 2. Populate
...
FROM (SELECT full_join_data_sv."KEY" AS "KEY", sum(CAST(full_join_data_sv.dot_prod1 AS FLOAT(53))) + 0.1654365360736847 AS dot_product 
FROM (SELECT kernel_input."KEY" AS "KEY", "SV_data_0_1".dual_coeff * exp(least(100.0, greatest(-100.0, -0.06541912694449836 * (power(kernel_input."Feature_0" - "SV_data_0_1".sv_0, 2) + power(kernel_input."Feature_1" - "SV_data_0_1".sv_1, 2) + power(kernel_input."Feature_2" - "SV_data_0_1".sv_2, 2) + power(kernel_input."Feature_3" - "SV_data_0_1".sv_3, 2))))) AS dot_prod1 
FROM kernel_input, "SV_data_0_1") AS full_join_data_sv GROUP BY full_join_data_sv."KEY") AS t), 
kernel_cte_0_1 AS 
(SELECT kernel_dp_0_1."KEY" AS "KEY", 0 AS "OVO_Class1", 1 AS "OVO_Class2", CASE WHEN (-kernel_dp_0_1.dot_product >= 0.0) THEN 1 ELSE 0 END AS "OVO_Decision", coalesce(-kernel_dp_0_1.dot_product, 1.0 / (1.0 + exp(least(100.0, greatest(-100.0, -(-((-kernel_dp_0_1.dot_product) * -3.3287813687304912 + 0.181239968530448))))))) AS "OVO_Conf" 
FROM kernel_dp_0_1)
 INSERT INTO "TMP_20221008175354_L6G14LW_KernAgg_B0" ("KEY", "OVO_Class1", "OVO_Class2", "OVO_Decision", "OVO_Conf") SELECT "T"."KEY", "T"."OVO_Class1", "T"."OVO_Class2", "T"."OVO_Decision", "T"."OVO_Conf" 
...
-- Code For temporary table TMP_20221008175354_5062UUP_KernAgg_B1 part 1. Create 


CREATE TEMPORARY TABLE "TMP_20221008175354_5062UUP_KernAgg_B1" (
	"KEY" BIGINT, 
	"OVO_Class1" BIGINT, 
	"OVO_Class2" BIGINT, 
	"OVO_Decision" BIGINT, 
...
-- Code For temporary table TMP_20221008175354_5062UUP_KernAgg_B1 part 2. Populate
...
FROM (SELECT full_join_data_sv_1."KEY" AS "KEY", sum(CAST(full_join_data_sv_1.dot_prod1 AS FLOAT(53))) + -0.06742919209178802 AS dot_product 
FROM (SELECT kernel_input."KEY" AS "KEY", "SV_data_0_2".dual_coeff * exp(least(100.0, greatest(-100.0, -0.06541912694449836 * (power(kernel_input."Feature_0" - "SV_data_0_2".sv_0, 2) + power(kernel_input."Feature_1" - "SV_data_0_2".sv_1, 2) + power(kernel_input."Feature_2" - "SV_data_0_2".sv_2, 2) + power(kernel_input."Feature_3" - "SV_data_0_2".sv_3, 2))))) AS dot_prod1 
FROM kernel_input, "SV_data_0_2") AS full_join_data_sv_1 GROUP BY full_join_data_sv_1."KEY") AS t_1), 
kernel_cte_0_2 AS 
(SELECT kernel_dp_0_2."KEY" AS "KEY", 0 AS "OVO_Class1", 2 AS "OVO_Class2", CASE WHEN (-kernel_dp_0_2.dot_product >= 0.0) THEN 1 ELSE 0 END AS "OVO_Decision", coalesce(-kernel_dp_0_2.dot_product, 1.0 / (1.0 + exp(least(100.0, greatest(-100.0, -(-((-kernel_dp_0_2.dot_product) * -3.406915176680882 + 0.03378828190210725))))))) AS "OVO_Conf" 
FROM kernel_dp_0_2)
 INSERT INTO "TMP_20221008175354_5062UUP_KernAgg_B1" ("KEY", "OVO_Class1", "OVO_Class2", "OVO_Decision", "OVO_Conf") SELECT "T"."KEY", "T"."OVO_Class1", "T"."OVO_Class2", "T"."OVO_Decision", "T"."OVO_Conf" 
...
-- Code For temporary table TMP_20221008175354_70ERO0R_KernAgg_B2 part 1. Create 


CREATE TEMPORARY TABLE "TMP_20221008175354_70ERO0R_KernAgg_B2" (
	"KEY" BIGINT, 
	"OVO_Class1" BIGINT, 
	"OVO_Class2" BIGINT, 
	"OVO_Decision" BIGINT, 
...
-- Code For temporary table TMP_20221008175354_70ERO0R_KernAgg_B2 part 2. Populate
...
FROM (SELECT full_join_data_sv_2."KEY" AS "KEY", sum(CAST(full_join_data_sv_2.dot_prod1 AS FLOAT(53))) + -0.11487892270088196 AS dot_product 
FROM (SELECT kernel_input."KEY" AS "KEY", "SV_data_1_2".dual_coeff * exp(least(100.0, greatest(-100.0, -0.06541912694449836 * (power(kernel_input."Feature_0" - "SV_data_1_2".sv_0, 2) + power(kernel_input."Feature_1" - "SV_data_1_2".sv_1, 2) + power(kernel_input."Feature_2" - "SV_data_1_2".sv_2, 2) + power(kernel_input."Feature_3" - "SV_data_1_2".sv_3, 2))))) AS dot_prod1 
FROM kernel_input, "SV_data_1_2") AS full_join_data_sv_2 GROUP BY full_join_data_sv_2."KEY") AS t_2), 
kernel_cte_1_2 AS 
(SELECT kernel_dp_1_2."KEY" AS "KEY", 1 AS "OVO_Class1", 2 AS "OVO_Class2", CASE WHEN (-kernel_dp_1_2.dot_product >= 0.0) THEN 1 ELSE 0 END AS "OVO_Decision", coalesce(-kernel_dp_1_2.dot_product, 1.0 / (1.0 + exp(least(100.0, greatest(-100.0, -(-((-kernel_dp_1_2.dot_product) * -4.062985968467786 + 0.13717507680501043))))))) AS "OVO_Conf" 
FROM kernel_dp_1_2)
 INSERT INTO "TMP_20221008175354_70ERO0R_KernAgg_B2" ("KEY", "OVO_Class1", "OVO_Class2", "OVO_Decision", "OVO_Conf") SELECT "T"."KEY", "T"."OVO_Class1", "T"."OVO_Class2", "T"."OVO_Decision", "T"."OVO_Conf" 
...
-- Code For temporary table TMP_20221008175354_NJNTFW8_KernAgg_agg part 1. Create 


CREATE TEMPORARY TABLE "TMP_20221008175354_NJNTFW8_KernAgg_agg" (
	"KEY" BIGINT NOT NULL, 
	"OVO_Vote_0" BIGINT, 
...
-- Code For temporary table TMP_20221008175354_NJNTFW8_KernAgg_agg part 2. Populate
...
FROM (SELECT "KernAgg_B0"."KEY" AS "KEY", "KernAgg_B0"."OVO_Class1" AS "OVO_Class1", "KernAgg_B0"."OVO_Class2" AS "OVO_Class2", "KernAgg_B0"."OVO_Decision" AS "OVO_Decision", "KernAgg_B0"."OVO_Conf" AS "OVO_Conf" 
FROM "TMP_20221008175354_L6G14LW_KernAgg_B0" AS "KernAgg_B0" UNION ALL SELECT "KernAgg_B1"."KEY" AS "KEY", "KernAgg_B1"."OVO_Class1" AS "OVO_Class1", "KernAgg_B1"."OVO_Class2" AS "OVO_Class2", "KernAgg_B1"."OVO_Decision" AS "OVO_Decision", "KernAgg_B1"."OVO_Conf" AS "OVO_Conf" 
FROM "TMP_20221008175354_5062UUP_KernAgg_B1" AS "KernAgg_B1" UNION ALL SELECT "KernAgg_B2"."KEY" AS "KEY", "KernAgg_B2"."OVO_Class1" AS "OVO_Class1", "KernAgg_B2"."OVO_Class2" AS "OVO_Class2", "KernAgg_B2"."OVO_Decision" AS "OVO_Decision", "KernAgg_B2"."OVO_Conf" AS "OVO_Conf" 
FROM "TMP_20221008175354_70ERO0R_KernAgg_B2" AS "KernAgg_B2") AS "KernAgg_EnsembleUnion"), 
"OVO_Votes" AS 
...
FROM "OVO_Votes", "MinMaxConf")
 INSERT INTO "TMP_20221008175354_NJNTFW8_KernAgg_agg" ("KEY", "OVO_Vote_0", "OVO_SumConfidence_0", "OVO_Vote_1", "OVO_SumConfidence_1", "OVO_Vote_2", "OVO_SumConfidence_2", max_abs_confidence, "OVO_Scale", "Score_0", "Score_1", "Score_2") SELECT "T"."KEY", "T"."OVO_Vote_0", "T"."OVO_SumConfidence_0", "T"."OVO_Vote_1", "T"."OVO_SumConfidence_1", "T"."OVO_Vote_2", "T"."OVO_SumConfidence_2", "T".max_abs_confidence, "T"."OVO_Scale", "T"."Score_0", "T"."Score_1", "T"."Score_2" 
...
-- Code For temporary table TMP_20221008175354_NJNTFW8_KernAgg_agg part 3. Create Index 

CREATE INDEX "ix_TMP_20221008175354_NJNTFW8_KernAgg_agg_KEY" ON "TMP_20221008175354_NJNTFW8_KernAgg_agg" ("KEY")
...
FROM "TMP_20221008175354_NJNTFW8_KernAgg_agg" AS "KernAgg_agg"), 
