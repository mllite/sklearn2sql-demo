...
(SELECT "ADS"."KEY" AS "KEY", 0.31645532248912833 * "ADS"."Feature_0" + 1.060601288628469 * "ADS"."Feature_1" + -1.6876770833907995 * "ADS"."Feature_2" + -0.7303910521504366 * "ADS"."Feature_3" + 0.245209504025 AS "Score_0", -0.4463801505071457 * "ADS"."Feature_0" + -0.2145912073577179 * "ADS"."Feature_1" + 0.6906544860947007 * "ADS"."Feature_2" + 0.25394645871317445 * "ADS"."Feature_3" + 0.120846515255 AS "Score_1", -0.5726329725146418 * "ADS"."Feature_0" + -1.4666772640920598 * "ADS"."Feature_1" + 1.3188808866747703 * "ADS"."Feature_2" + 0.7108743238716934 * "ADS"."Feature_3" + -0.492911135743 AS "Score_2" 
FROM iris "ADS"), 
orig_cte AS 
(SELECT linear_model_cte."KEY" AS "KEY", linear_model_cte."Score_0" AS "Score_0", linear_model_cte."Score_1" AS "Score_1", linear_model_cte."Score_2" AS "Score_2", (1.0 / (1.0 + exp(-linear_model_cte."Score_0"))) / (1.0 / (1.0 + exp(-linear_model_cte."Score_0")) + 1.0 / (1.0 + exp(-linear_model_cte."Score_1")) + 1.0 / (1.0 + exp(-linear_model_cte."Score_2"))) AS "Proba_0", (1.0 / (1.0 + exp(-linear_model_cte."Score_1"))) / (1.0 / (1.0 + exp(-linear_model_cte."Score_0")) + 1.0 / (1.0 + exp(-linear_model_cte."Score_1")) + 1.0 / (1.0 + exp(-linear_model_cte."Score_2"))) AS "Proba_1", (1.0 / (1.0 + exp(-linear_model_cte."Score_2"))) / (1.0 / (1.0 + exp(-linear_model_cte."Score_0")) + 1.0 / (1.0 + exp(-linear_model_cte."Score_1")) + 1.0 / (1.0 + exp(-linear_model_cte."Score_2"))) AS "Proba_2", CAST(NULL AS FLOAT) AS "LogProba_0", CAST(NULL AS FLOAT) AS "LogProba_1", CAST(NULL AS FLOAT) AS "LogProba_2", CAST(NULL AS INTEGER) AS "Decision" 
