...
(SELECT `multinomial_cte`.`KEY` AS `KEY`, 0.32 AS `Proba_0`, CASE WHEN (0.32 IS NULL OR 0.32 > 0.0) THEN log(0.32) ELSE -1.79769313486231e+308 END AS `LogProba_0`, 0.68 AS `Proba_1`, CASE WHEN (0.68 IS NULL OR 0.68 > 0.0) THEN log(0.68) ELSE -1.79769313486231e+308 END AS `LogProba_1`, `multinomial_cte`.`multinomial_expr` AS `Decision` 
