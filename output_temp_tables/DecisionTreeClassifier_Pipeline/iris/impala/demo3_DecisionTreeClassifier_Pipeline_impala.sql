...
-- Code For temporary table tmp_20180602152720_izm_ads_imp_1_out part 1. Create 


CREATE TABLE tmp_20180602152720_izm_ads_imp_1_out (
	`KEY` BIGINT, 
...
-- Code For temporary table tmp_20180602152720_izm_ads_imp_1_out part 2. Populate

INSERT INTO tmp_20180602152720_izm_ads_imp_1_out (`KEY`, imputer_output_2, imputer_output_3, imputer_output_4, imputer_output_5) SELECT `ADS_imp_1_OUT`.`KEY`, `ADS_imp_1_OUT`.imputer_output_2, `ADS_imp_1_OUT`.imputer_output_3, `ADS_imp_1_OUT`.imputer_output_4, `ADS_imp_1_OUT`.imputer_output_5 
FROM (SELECT `ADS`.`KEY` AS `KEY`, CASE WHEN (`ADS`.`Feature_0` IS NULL) THEN 5.8474999999999975 ELSE `ADS`.`Feature_0` END AS imputer_output_2, CASE WHEN (`ADS`.`Feature_1` IS NULL) THEN 3.0366666666666657 ELSE `ADS`.`Feature_1` END AS imputer_output_3, CASE WHEN (`ADS`.`Feature_2` IS NULL) THEN 3.8450000000000006 ELSE `ADS`.`Feature_2` END AS imputer_output_4, CASE WHEN (`ADS`.`Feature_3` IS NULL) THEN 1.245 ELSE `ADS`.`Feature_3` END AS imputer_output_5 
FROM iris AS `ADS`) AS `ADS_imp_1_OUT`

-- Code For temporary table tmp_20180602152720_xhl_ads_sca_2_out part 1. Create 


CREATE TABLE tmp_20180602152720_xhl_ads_sca_2_out (
	`KEY` BIGINT, 
...
-- Code For temporary table tmp_20180602152720_xhl_ads_sca_2_out part 2. Populate

INSERT INTO tmp_20180602152720_xhl_ads_sca_2_out (`KEY`, scaler_output_2, scaler_output_3, scaler_output_4, scaler_output_5) SELECT `ADS_sca_2_OUT`.`KEY`, `ADS_sca_2_OUT`.scaler_output_2, `ADS_sca_2_OUT`.scaler_output_3, `ADS_sca_2_OUT`.scaler_output_4, `ADS_sca_2_OUT`.scaler_output_5 
...
FROM tmp_20180602152720_izm_ads_imp_1_out AS `ADS_imp_1_OUT`) AS `ADS_sca_2_OUT`
...
FROM tmp_20180602152720_xhl_ads_sca_2_out AS `ADS_sca_2_OUT`), 
`DT_node_data` AS 
