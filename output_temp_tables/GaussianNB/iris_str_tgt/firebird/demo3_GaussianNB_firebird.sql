...
-- Code For temporary table TMP_20180602_MYX_NAIVEBAYES_SCO part 1. Create 


CREATE GLOBAL TEMPORARY TABLE "TMP_20180602_MYX_NAIVEBAYES_SCO" (
	"KEY" BIGINT NOT NULL, 
...
-- Code For temporary table TMP_20180602_MYX_NAIVEBAYES_SCO part 2. Populate

INSERT INTO "TMP_20180602_MYX_NAIVEBAYES_SCO" ("KEY", "Score_CLASS_0", "Score_CLASS_1", "Score_CLASS_2") SELECT "U"."KEY", "U"."Score_CLASS_0", "U"."Score_CLASS_1", "U"."Score_CLASS_2" 
FROM (WITH centered_data AS 
(SELECT "ADS"."KEY" AS "KEY", CAST("ADS"."Feature_0" AS DOUBLE PRECISION) - 4.95945945945946 AS "Feature_0_CLASS_0", CAST("ADS"."Feature_1" AS DOUBLE PRECISION) - 3.4162162162162164 AS "Feature_1_CLASS_0", CAST("ADS"."Feature_2" AS DOUBLE PRECISION) - 1.4864864864864862 AS "Feature_2_CLASS_0", CAST("ADS"."Feature_3" AS DOUBLE PRECISION) - 0.2594594594594595 AS "Feature_3_CLASS_0", CAST("ADS"."Feature_0" AS DOUBLE PRECISION) - 5.914999999999999 AS "Feature_0_CLASS_1", CAST("ADS"."Feature_1" AS DOUBLE PRECISION) - 2.7600000000000007 AS "Feature_1_CLASS_1", CAST("ADS"."Feature_2" AS DOUBLE PRECISION) - 4.245 AS "Feature_2_CLASS_1", CAST("ADS"."Feature_3" AS DOUBLE PRECISION) - 1.325 AS "Feature_3_CLASS_1", CAST("ADS"."Feature_0" AS DOUBLE PRECISION) - 6.548837209302325 AS "Feature_0_CLASS_2", CAST("ADS"."Feature_1" AS DOUBLE PRECISION) - 2.9674418604651165 AS "Feature_1_CLASS_2", CAST("ADS"."Feature_2" AS DOUBLE PRECISION) - 5.502325581395348 AS "Feature_2_CLASS_2", CAST("ADS"."Feature_3" AS DOUBLE PRECISION) - 2.01860465116279 AS "Feature_3_CLASS_2" 
...
FROM (SELECT centered_data."KEY" AS "KEY", 0 AS "Feature", -0.5 * -0.27583381930241424 - (0.5 * centered_data."Feature_0_CLASS_0" * centered_data."Feature_0_CLASS_0") / 0.12078889992858824 AS "log_proba_CLASS_0", -0.5 * 0.607816937322572 - (0.5 * centered_data."Feature_0_CLASS_1" * centered_data."Feature_0_CLASS_1") / 0.29227500292347514 AS "log_proba_CLASS_1", -0.5 * 0.8115275892651813 - (0.5 * centered_data."Feature_0_CLASS_2" * centered_data."Feature_0_CLASS_2") / 0.35831260432964057 AS "log_proba_CLASS_2" 
FROM centered_data UNION ALL SELECT centered_data."KEY" AS "KEY", 1 AS "Feature", -0.5 * -0.04133744763057782 - (0.5 * centered_data."Feature_1_CLASS_0" * centered_data."Feature_1_CLASS_0") / 0.1527100102280769 AS "log_proba_CLASS_0", -0.5 * -0.38867331321440274 - (0.5 * centered_data."Feature_1_CLASS_1" * centered_data."Feature_1_CLASS_1") / 0.107900002923475 AS "log_proba_CLASS_1", -0.5 * -0.4706749256464802 - (0.5 * centered_data."Feature_1_CLASS_2" * centered_data."Feature_1_CLASS_2") / 0.09940508675257179 AS "log_proba_CLASS_2" 
FROM centered_data UNION ALL SELECT centered_data."KEY" AS "KEY", 2 AS "Feature", -0.5 * -1.7899394757771134 - (0.5 * centered_data."Feature_2_CLASS_0" * centered_data."Feature_2_CLASS_0") / 0.02657414463275185 AS "log_proba_CLASS_0", -0.5 * 0.3680924075025722 - (0.5 * centered_data."Feature_2_CLASS_1" * centered_data."Feature_2_CLASS_1") / 0.22997500292347509 AS "log_proba_CLASS_1", -0.5 * 0.40976870412290883 - (0.5 * centered_data."Feature_2_CLASS_2" * centered_data."Feature_2_CLASS_2") / 0.2397620364551138 AS "log_proba_CLASS_2" 
FROM centered_data UNION ALL SELECT centered_data."KEY" AS "KEY", 3 AS "Feature", -0.5 * -2.529789819103305 - (0.5 * centered_data."Feature_3_CLASS_0" * centered_data."Feature_3_CLASS_0") / 0.012680791820480103 AS "log_proba_CLASS_0", -0.5 * -1.3233195778816191 - (0.5 * centered_data."Feature_3_CLASS_1" * centered_data."Feature_3_CLASS_1") / 0.04237500292347501 AS "log_proba_CLASS_1", -0.5 * -0.703934755097896 - (0.5 * centered_data."Feature_3_CLASS_2" * centered_data."Feature_3_CLASS_2") / 0.07872363732044632 AS "log_proba_CLASS_2" 
...
-- Code For temporary table TMP_20180602_MYX_NAIVEBAYES_SCO part 3. Create Index 

CREATE INDEX "ix_TMP_20180602_MYX_NAI_1d64" ON "TMP_20180602_MYX_NAIVEBAYES_SCO" ("KEY")
...
FROM "TMP_20180602_MYX_NAIVEBAYES_SCO" AS "NaiveBayes_Scores"), 
