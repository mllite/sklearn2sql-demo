...
-- Code For temporary table TMP_20180602_06M_KERNAGG_B0 part 1. Create 


CREATE GLOBAL TEMPORARY TABLE "TMP_20180602_06M_KERNAGG_B0" (
	"KEY" BIGINT, 
	"OVO_Class1" BIGINT, 
	"OVO_Class2" BIGINT, 
	"OVO_Decision" BIGINT, 
...
-- Code For temporary table TMP_20180602_06M_KERNAGG_B0 part 2. Populate

INSERT INTO "TMP_20180602_06M_KERNAGG_B0" ("KEY", "OVO_Class1", "OVO_Class2", "OVO_Decision", "OVO_Conf") SELECT "U"."KEY", "U"."OVO_Class1", "U"."OVO_Class2", "U"."OVO_Decision", "U"."OVO_Conf" 
...
FROM (SELECT 0 AS sv_idx, 0.0 AS dual_coeff, 4.8 AS sv_0, 3.4 AS sv_1, 1.9 AS sv_2, 0.2 AS sv_3 FROM rdb$database UNION ALL SELECT 1 AS sv_idx, 0.00888416756170261 AS dual_coeff, 5.1 AS sv_0, 3.3 AS sv_1, 1.7 AS sv_2, 0.5 AS sv_3 FROM rdb$database UNION ALL SELECT 2 AS sv_idx, 0.0 AS dual_coeff, 5.1 AS sv_0, 3.8 AS sv_1, 1.9 AS sv_2, 0.4 AS sv_3 FROM rdb$database UNION ALL SELECT 3 AS sv_idx, 0.0005109388709751139 AS dual_coeff, 4.5 AS sv_0, 2.3 AS sv_1, 1.3 AS sv_2, 0.3 AS sv_3 FROM rdb$database UNION ALL SELECT 4 AS sv_idx, -0.00939510643267772 AS dual_coeff, 5.1 AS sv_0, 2.5 AS sv_1, 3.0 AS sv_2, 1.1 AS sv_3 FROM rdb$database UNION ALL SELECT 5 AS sv_idx, -0.0 AS dual_coeff, 5.9 AS sv_0, 3.2 AS sv_1, 4.8 AS sv_2, 1.8 AS sv_3 FROM rdb$database UNION ALL SELECT 6 AS sv_idx, -0.0 AS dual_coeff, 6.3 AS sv_0, 2.5 AS sv_1, 4.9 AS sv_2, 1.5 AS sv_3 FROM rdb$database) AS "Values"), 
kernel_dp_0_1 AS 
(SELECT t."KEY" AS "KEY", t.dot_product AS dot_product 
FROM (SELECT full_join_data_sv."KEY" AS "KEY", sum(CAST(full_join_data_sv.dot_prod1 AS DOUBLE PRECISION)) + 1.1361099459962782 AS dot_product 
...
kernel_cte_0_1 AS 
(SELECT kernel_dp_0_1."KEY" AS "KEY", 0 AS "OVO_Class1", 1 AS "OVO_Class2", CASE WHEN (-kernel_dp_0_1.dot_product >= 0.0) THEN 1 ELSE 0 END AS "OVO_Decision", coalesce(-kernel_dp_0_1.dot_product, 1.0 / (1.0 + exp(CASE WHEN (minvalue(maxvalue(-100.0, -(-((-kernel_dp_0_1.dot_product) * -1.6568593205688362 + -0.9482594418264659))), 100.0) >= -709.782712893384) THEN minvalue(maxvalue(-100.0, -(-((-kernel_dp_0_1.dot_product) * -1.6568593205688362 + -0.9482594418264659))), 100.0) ELSE -709.782712893384 END))) AS "OVO_Conf" 
FROM kernel_dp_0_1)
...
-- Code For temporary table TMP_20180602_OQ3_KERNAGG_B1 part 1. Create 


CREATE GLOBAL TEMPORARY TABLE "TMP_20180602_OQ3_KERNAGG_B1" (
	"KEY" BIGINT, 
	"OVO_Class1" BIGINT, 
	"OVO_Class2" BIGINT, 
	"OVO_Decision" BIGINT, 
...
-- Code For temporary table TMP_20180602_OQ3_KERNAGG_B1 part 2. Populate

INSERT INTO "TMP_20180602_OQ3_KERNAGG_B1" ("KEY", "OVO_Class1", "OVO_Class2", "OVO_Decision", "OVO_Conf") SELECT "U"."KEY", "U"."OVO_Class1", "U"."OVO_Class2", "U"."OVO_Decision", "U"."OVO_Conf" 
FROM (WITH kernel_input AS 
(SELECT "ADS"."KEY" AS "KEY", CAST("ADS"."Feature_0" AS DOUBLE PRECISION) AS "Feature_0", CAST("ADS"."Feature_1" AS DOUBLE PRECISION) AS "Feature_1", CAST("ADS"."Feature_2" AS DOUBLE PRECISION) AS "Feature_2", CAST("ADS"."Feature_3" AS DOUBLE PRECISION) AS "Feature_3", CAST("ADS"."TGT" AS DOUBLE PRECISION) AS "TGT" 
FROM iris AS "ADS"), 
"SV_data_0_2" AS 
(SELECT "Values".sv_idx AS sv_idx, CAST("Values".dual_coeff AS DOUBLE PRECISION) AS dual_coeff, CAST("Values".sv_0 AS DOUBLE PRECISION) AS sv_0, CAST("Values".sv_1 AS DOUBLE PRECISION) AS sv_1, CAST("Values".sv_2 AS DOUBLE PRECISION) AS sv_2, CAST("Values".sv_3 AS DOUBLE PRECISION) AS sv_3 
FROM (SELECT 0 AS sv_idx, 0.0006286056310521933 AS dual_coeff, 4.8 AS sv_0, 3.4 AS sv_1, 1.9 AS sv_2, 0.2 AS sv_3 FROM rdb$database UNION ALL SELECT 1 AS sv_idx, 0.0005066779305344128 AS dual_coeff, 5.1 AS sv_0, 3.3 AS sv_1, 1.7 AS sv_2, 0.5 AS sv_3 FROM rdb$database UNION ALL SELECT 2 AS sv_idx, 0.0006814761767680465 AS dual_coeff, 5.1 AS sv_0, 3.8 AS sv_1, 1.9 AS sv_2, 0.4 AS sv_3 FROM rdb$database UNION ALL SELECT 3 AS sv_idx, 0.0 AS dual_coeff, 4.5 AS sv_0, 2.3 AS sv_1, 1.3 AS sv_2, 0.3 AS sv_3 FROM rdb$database UNION ALL SELECT 7 AS sv_idx, -0.0 AS dual_coeff, 6.1 AS sv_0, 3.0 AS sv_1, 4.9 AS sv_2, 1.8 AS sv_3 FROM rdb$database UNION ALL SELECT 8 AS sv_idx, -0.0018167597383546528 AS dual_coeff, 4.9 AS sv_0, 2.5 AS sv_1, 4.5 AS sv_2, 1.7 AS sv_3 FROM rdb$database UNION ALL SELECT 9 AS sv_idx, -0.0 AS dual_coeff, 6.0 AS sv_0, 3.0 AS sv_1, 4.8 AS sv_2, 1.8 AS sv_3 FROM rdb$database UNION ALL SELECT 10 AS sv_idx, -0.0 AS dual_coeff, 6.3 AS sv_0, 2.8 AS sv_1, 5.1 AS sv_2, 1.5 AS sv_3 FROM rdb$database) AS "Values"), 
kernel_dp_0_2 AS 
(SELECT t_1."KEY" AS "KEY", t_1.dot_product AS dot_product 
FROM (SELECT full_join_data_sv_1."KEY" AS "KEY", sum(CAST(full_join_data_sv_1.dot_prod1 AS DOUBLE PRECISION)) + 1.1844509506518808 AS dot_product 
...
kernel_cte_0_2 AS 
(SELECT kernel_dp_0_2."KEY" AS "KEY", 0 AS "OVO_Class1", 2 AS "OVO_Class2", CASE WHEN (-kernel_dp_0_2.dot_product >= 0.0) THEN 1 ELSE 0 END AS "OVO_Decision", coalesce(-kernel_dp_0_2.dot_product, 1.0 / (1.0 + exp(CASE WHEN (minvalue(maxvalue(-100.0, -(-((-kernel_dp_0_2.dot_product) * -1.9912706169429077 + -0.7665376243657053))), 100.0) >= -709.782712893384) THEN minvalue(maxvalue(-100.0, -(-((-kernel_dp_0_2.dot_product) * -1.9912706169429077 + -0.7665376243657053))), 100.0) ELSE -709.782712893384 END))) AS "OVO_Conf" 
FROM kernel_dp_0_2)
...
-- Code For temporary table TMP_20180602_5ZX_KERNAGG_B2 part 1. Create 


CREATE GLOBAL TEMPORARY TABLE "TMP_20180602_5ZX_KERNAGG_B2" (
	"KEY" BIGINT, 
	"OVO_Class1" BIGINT, 
	"OVO_Class2" BIGINT, 
	"OVO_Decision" BIGINT, 
...
-- Code For temporary table TMP_20180602_5ZX_KERNAGG_B2 part 2. Populate

INSERT INTO "TMP_20180602_5ZX_KERNAGG_B2" ("KEY", "OVO_Class1", "OVO_Class2", "OVO_Decision", "OVO_Conf") SELECT "U"."KEY", "U"."OVO_Class1", "U"."OVO_Class2", "U"."OVO_Decision", "U"."OVO_Conf" 
FROM (WITH kernel_input AS 
(SELECT "ADS"."KEY" AS "KEY", CAST("ADS"."Feature_0" AS DOUBLE PRECISION) AS "Feature_0", CAST("ADS"."Feature_1" AS DOUBLE PRECISION) AS "Feature_1", CAST("ADS"."Feature_2" AS DOUBLE PRECISION) AS "Feature_2", CAST("ADS"."Feature_3" AS DOUBLE PRECISION) AS "Feature_3", CAST("ADS"."TGT" AS DOUBLE PRECISION) AS "TGT" 
FROM iris AS "ADS"), 
"SV_data_1_2" AS 
(SELECT "Values".sv_idx AS sv_idx, CAST("Values".dual_coeff AS DOUBLE PRECISION) AS dual_coeff, CAST("Values".sv_0 AS DOUBLE PRECISION) AS sv_0, CAST("Values".sv_1 AS DOUBLE PRECISION) AS sv_1, CAST("Values".sv_2 AS DOUBLE PRECISION) AS sv_2, CAST("Values".sv_3 AS DOUBLE PRECISION) AS sv_3 
FROM (SELECT 4 AS sv_idx, 0.0 AS dual_coeff, 5.1 AS sv_0, 2.5 AS sv_1, 3.0 AS sv_2, 1.1 AS sv_3 FROM rdb$database UNION ALL SELECT 5 AS sv_idx, 1.0 AS dual_coeff, 5.9 AS sv_0, 3.2 AS sv_1, 4.8 AS sv_2, 1.8 AS sv_3 FROM rdb$database UNION ALL SELECT 6 AS sv_idx, 0.6261843191965794 AS dual_coeff, 6.3 AS sv_0, 2.5 AS sv_1, 4.9 AS sv_2, 1.5 AS sv_3 FROM rdb$database UNION ALL SELECT 7 AS sv_idx, -0.07792039423656584 AS dual_coeff, 6.1 AS sv_0, 3.0 AS sv_1, 4.9 AS sv_2, 1.8 AS sv_3 FROM rdb$database UNION ALL SELECT 8 AS sv_idx, -0.0995148175960605 AS dual_coeff, 4.9 AS sv_0, 2.5 AS sv_1, 4.5 AS sv_2, 1.7 AS sv_3 FROM rdb$database UNION ALL SELECT 9 AS sv_idx, -1.0 AS dual_coeff, 6.0 AS sv_0, 3.0 AS sv_1, 4.8 AS sv_2, 1.8 AS sv_3 FROM rdb$database UNION ALL SELECT 10 AS sv_idx, -0.4487491073639533 AS dual_coeff, 6.3 AS sv_0, 2.8 AS sv_1, 5.1 AS sv_2, 1.5 AS sv_3 FROM rdb$database) AS "Values"), 
kernel_dp_1_2 AS 
(SELECT t_2."KEY" AS "KEY", t_2.dot_product AS dot_product 
FROM (SELECT full_join_data_sv_2."KEY" AS "KEY", sum(CAST(full_join_data_sv_2.dot_prod1 AS DOUBLE PRECISION)) + 15.184809946631539 AS dot_product 
...
kernel_cte_1_2 AS 
(SELECT kernel_dp_1_2."KEY" AS "KEY", 1 AS "OVO_Class1", 2 AS "OVO_Class2", CASE WHEN (-kernel_dp_1_2.dot_product >= 0.0) THEN 1 ELSE 0 END AS "OVO_Decision", coalesce(-kernel_dp_1_2.dot_product, 1.0 / (1.0 + exp(CASE WHEN (minvalue(maxvalue(-100.0, -(-((-kernel_dp_1_2.dot_product) * -0.31066041880700385 + 0.14483676971616083))), 100.0) >= -709.782712893384) THEN minvalue(maxvalue(-100.0, -(-((-kernel_dp_1_2.dot_product) * -0.31066041880700385 + 0.14483676971616083))), 100.0) ELSE -709.782712893384 END))) AS "OVO_Conf" 
FROM kernel_dp_1_2)
...
-- Code For temporary table TMP_20180602_UY0_KERNAGG_AGG part 1. Create 


CREATE GLOBAL TEMPORARY TABLE "TMP_20180602_UY0_KERNAGG_AGG" (
	"KEY" BIGINT NOT NULL, 
	"OVO_Vote_0" BIGINT, 
...
FROM "TMP_20180602_06M_KERNAGG_B0" AS "KernAgg_B0" UNION ALL SELECT "KernAgg_B1"."KEY" AS "KEY", "KernAgg_B1"."OVO_Class1" AS "OVO_Class1", "KernAgg_B1"."OVO_Class2" AS "OVO_Class2", "KernAgg_B1"."OVO_Decision" AS "OVO_Decision", "KernAgg_B1"."OVO_Conf" AS "OVO_Conf" 
FROM "TMP_20180602_OQ3_KERNAGG_B1" AS "KernAgg_B1" UNION ALL SELECT "KernAgg_B2"."KEY" AS "KEY", "KernAgg_B2"."OVO_Class1" AS "OVO_Class1", "KernAgg_B2"."OVO_Class2" AS "OVO_Class2", "KernAgg_B2"."OVO_Decision" AS "OVO_Decision", "KernAgg_B2"."OVO_Conf" AS "OVO_Conf" 
FROM "TMP_20180602_5ZX_KERNAGG_B2" AS "KernAgg_B2") AS "KernAgg_EnsembleUnion"), 
"OVO_Votes" AS 
...
-- Code For temporary table TMP_20180602_UY0_KERNAGG_AGG part 3. Create Index 

CREATE INDEX "ix_TMP_20180602_UY0_KER_790a" ON "TMP_20180602_UY0_KERNAGG_AGG" ("KEY")
...
FROM "TMP_20180602_UY0_KERNAGG_AGG" AS "KernAgg_agg"), 
