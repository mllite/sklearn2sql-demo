...
"ADS_sca_2_OUT" AS 
(SELECT "ADS_imp_1_OUT"."KEY" AS "KEY", (CAST("ADS_imp_1_OUT".impter_2 AS FLOAT) - 0.125979920549) / 0.867636205438 AS scaler_2, (CAST("ADS_imp_1_OUT".impter_3 AS FLOAT) - 0.010432834372) / 0.997787454055 AS scaler_3, (CAST("ADS_imp_1_OUT".impter_4 AS FLOAT) - -0.0638443987193) / 0.931068352722 AS scaler_4, (CAST("ADS_imp_1_OUT".impter_5 AS FLOAT) - 0.0572335197161) / 0.893967294087 AS scaler_5, (CAST("ADS_imp_1_OUT".impter_6 AS FLOAT) - 0.15191881742) / 0.966528132881 AS scaler_6, (CAST("ADS_imp_1_OUT".impter_7 AS FLOAT) - 0.00442103774357) / 0.930210191267 AS scaler_7, (CAST("ADS_imp_1_OUT".impter_8 AS FLOAT) - 0.146711319319) / 0.932495044936 AS scaler_8, (CAST("ADS_imp_1_OUT".impter_9 AS FLOAT) - -0.0406840467287) / 1.00506993354 AS scaler_9, (CAST("ADS_imp_1_OUT".impter_10 AS FLOAT) - 0.0266827679184) / 1.02582451068 AS scaler_10, (CAST("ADS_imp_1_OUT".impter_11 AS FLOAT) - -0.0128582338004) / 0.891533152315 AS scaler_11 
FROM "ADS_imp_1_OUT"), 
"IL" AS 
