...
FROM (SELECT 0 AS sv_idx, 0.00226455961707 AS dual_coeff, 5.1 AS sv_0, 3.8 AS sv_1, 1.9 AS sv_2, 0.4 AS sv_3 UNION ALL SELECT 1 AS sv_idx, 0.00273521240728 AS dual_coeff, 4.5 AS sv_0, 2.3 AS sv_1, 1.3 AS sv_2, 0.3 AS sv_3 UNION ALL SELECT 2 AS sv_idx, 0.000138948014128 AS dual_coeff, 5.4 AS sv_0, 3.4 AS sv_1, 1.7 AS sv_2, 0.2 AS sv_3 UNION ALL SELECT 3 AS sv_idx, -0.0 AS dual_coeff, 6.3 AS sv_0, 2.5 AS sv_1, 4.9 AS sv_2, 1.5 AS sv_3 UNION ALL SELECT 4 AS sv_idx, -0.00513872003847 AS dual_coeff, 5.0 AS sv_0, 2.0 AS sv_1, 3.5 AS sv_2, 1.0 AS sv_3) AS "Values"), 
kernel_dp_0_1 AS 
(SELECT t."KEY" AS "KEY", t.dot_product AS dot_product 
FROM (SELECT full_join_data_sv."KEY" AS "KEY", sum(full_join_data_sv.dot_prod1) + 1.23345083318 AS dot_product 
FROM (SELECT "ADS"."KEY" AS "KEY", "SV_data_0_1".dual_coeff * power(0.25 * ("ADS"."Feature_0" * "SV_data_0_1".sv_0 + "ADS"."Feature_1" * "SV_data_0_1".sv_1 + "ADS"."Feature_2" * "SV_data_0_1".sv_2 + "ADS"."Feature_3" * "SV_data_0_1".sv_3) + 0.0, 3) AS dot_prod1 
FROM iris AS "ADS", "SV_data_0_1") AS full_join_data_sv GROUP BY full_join_data_sv."KEY") AS t), 
kernel_cte_0_1 AS 
(SELECT kernel_dp_0_1."KEY" AS "KEY", 0 AS "OVO_Class1", 1 AS "OVO_Class2", CASE WHEN (-kernel_dp_0_1.dot_product > 0.0) THEN 1 ELSE 0 END AS "OVO_Decision", coalesce(-kernel_dp_0_1.dot_product, 1.0 / (1.0 + exp(CASE WHEN ((-kernel_dp_0_1.dot_product) * -1.40530845443 + -0.601123839776 >= -709.782712893) THEN (-kernel_dp_0_1.dot_product) * -1.40530845443 + -0.601123839776 ELSE -709.782712893 END))) AS "OVO_Confidence" 
FROM kernel_dp_0_1), 
"SV_data_0_2" AS 
(SELECT "Values".sv_idx AS sv_idx, "Values".dual_coeff AS dual_coeff, "Values".sv_0 AS sv_0, "Values".sv_1 AS sv_1, "Values".sv_2 AS sv_2, "Values".sv_3 AS sv_3 
FROM (SELECT 0 AS sv_idx, 0.000771145367011 AS dual_coeff, 5.1 AS sv_0, 3.8 AS sv_1, 1.9 AS sv_2, 0.4 AS sv_3 UNION ALL SELECT 1 AS sv_idx, 0.0 AS dual_coeff, 4.5 AS sv_0, 2.3 AS sv_1, 1.3 AS sv_2, 0.3 AS sv_3 UNION ALL SELECT 2 AS sv_idx, 0.0 AS dual_coeff, 5.4 AS sv_0, 3.4 AS sv_1, 1.7 AS sv_2, 0.2 AS sv_3 UNION ALL SELECT 5 AS sv_idx, -0.0 AS dual_coeff, 7.2 AS sv_0, 3.0 AS sv_1, 5.8 AS sv_2, 1.6 AS sv_3 UNION ALL SELECT 6 AS sv_idx, -0.000771145367011 AS dual_coeff, 6.0 AS sv_0, 2.2 AS sv_1, 5.0 AS sv_2, 1.5 AS sv_3 UNION ALL SELECT 7 AS sv_idx, -0.0 AS dual_coeff, 6.3 AS sv_0, 2.5 AS sv_1, 5.0 AS sv_2, 1.9 AS sv_3) AS "Values"), 
kernel_dp_0_2 AS 
(SELECT t_1."KEY" AS "KEY", t_1.dot_product AS dot_product 
FROM (SELECT full_join_data_sv_1."KEY" AS "KEY", sum(full_join_data_sv_1.dot_prod1) + 1.38091632856 AS dot_product 
FROM (SELECT "ADS"."KEY" AS "KEY", "SV_data_0_2".dual_coeff * power(0.25 * ("ADS"."Feature_0" * "SV_data_0_2".sv_0 + "ADS"."Feature_1" * "SV_data_0_2".sv_1 + "ADS"."Feature_2" * "SV_data_0_2".sv_2 + "ADS"."Feature_3" * "SV_data_0_2".sv_3) + 0.0, 3) AS dot_prod1 
FROM iris AS "ADS", "SV_data_0_2") AS full_join_data_sv_1 GROUP BY full_join_data_sv_1."KEY") AS t_1), 
kernel_cte_0_2 AS 
(SELECT kernel_dp_0_2."KEY" AS "KEY", 0 AS "OVO_Class1", 2 AS "OVO_Class2", CASE WHEN (-kernel_dp_0_2.dot_product > 0.0) THEN 1 ELSE 0 END AS "OVO_Decision", coalesce(-kernel_dp_0_2.dot_product, 1.0 / (1.0 + exp(CASE WHEN ((-kernel_dp_0_2.dot_product) * -1.46053597418 + -0.786372747889 >= -709.782712893) THEN (-kernel_dp_0_2.dot_product) * -1.46053597418 + -0.786372747889 ELSE -709.782712893 END))) AS "OVO_Confidence" 
FROM kernel_dp_0_2), 
"SV_data_1_2" AS 
(SELECT "Values".sv_idx AS sv_idx, "Values".dual_coeff AS dual_coeff, "Values".sv_0 AS sv_0, "Values".sv_1 AS sv_1, "Values".sv_2 AS sv_2, "Values".sv_3 AS sv_3 
FROM (SELECT 3 AS sv_idx, 0.112582889627 AS dual_coeff, 6.3 AS sv_0, 2.5 AS sv_1, 4.9 AS sv_2, 1.5 AS sv_3 UNION ALL SELECT 4 AS sv_idx, 0.0 AS dual_coeff, 5.0 AS sv_0, 2.0 AS sv_1, 3.5 AS sv_2, 1.0 AS sv_3 UNION ALL SELECT 5 AS sv_idx, -0.00960120233735 AS dual_coeff, 7.2 AS sv_0, 3.0 AS sv_1, 5.8 AS sv_2, 1.6 AS sv_3 UNION ALL SELECT 6 AS sv_idx, -0.0714566390298 AS dual_coeff, 6.0 AS sv_0, 2.2 AS sv_1, 5.0 AS sv_2, 1.5 AS sv_3 UNION ALL SELECT 7 AS sv_idx, -0.0315250482601 AS dual_coeff, 6.3 AS sv_0, 2.5 AS sv_1, 5.0 AS sv_2, 1.9 AS sv_3) AS "Values"), 
kernel_dp_1_2 AS 
(SELECT t_2."KEY" AS "KEY", t_2.dot_product AS dot_product 
FROM (SELECT full_join_data_sv_2."KEY" AS "KEY", sum(full_join_data_sv_2.dot_prod1) + 4.53219328975 AS dot_product 
FROM (SELECT "ADS"."KEY" AS "KEY", "SV_data_1_2".dual_coeff * power(0.25 * ("ADS"."Feature_0" * "SV_data_1_2".sv_0 + "ADS"."Feature_1" * "SV_data_1_2".sv_1 + "ADS"."Feature_2" * "SV_data_1_2".sv_2 + "ADS"."Feature_3" * "SV_data_1_2".sv_3) + 0.0, 3) AS dot_prod1 
FROM iris AS "ADS", "SV_data_1_2") AS full_join_data_sv_2 GROUP BY full_join_data_sv_2."KEY") AS t_2), 
kernel_cte_1_2 AS 
(SELECT kernel_dp_1_2."KEY" AS "KEY", 1 AS "OVO_Class1", 2 AS "OVO_Class2", CASE WHEN (-kernel_dp_1_2.dot_product > 0.0) THEN 1 ELSE 0 END AS "OVO_Decision", coalesce(-kernel_dp_1_2.dot_product, 1.0 / (1.0 + exp(CASE WHEN ((-kernel_dp_1_2.dot_product) * -0.740081890059 + 0.092732113649 >= -709.782712893) THEN (-kernel_dp_1_2.dot_product) * -0.740081890059 + 0.092732113649 ELSE -709.782712893 END))) AS "OVO_Confidence" 
FROM kernel_dp_1_2), 
