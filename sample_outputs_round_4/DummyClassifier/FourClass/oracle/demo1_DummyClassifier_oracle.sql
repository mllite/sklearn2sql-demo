...
FROM "INPUT_DATA" "ADS"), 
multinomial_cte AS 
(SELECT random_number_cte."KEY" AS "KEY", CASE WHEN (random_number_cte.uniform <= 0.25) THEN 0 ELSE CASE WHEN (random_number_cte.uniform <= 0.5) THEN 1 ELSE CASE WHEN (random_number_cte.uniform <= 0.74) THEN 2 ELSE CASE WHEN (random_number_cte.uniform <= 1.0) THEN 3 END END END END AS multinomial_expr 
FROM random_number_cte), 
dummy_class_reg_cte AS 
(SELECT multinomial_cte."KEY" AS "KEY", 0.25 AS "Proba_0", CASE WHEN (0.25 IS NULL OR 0.25 > 0.0) THEN ln(0.25) ELSE -BINARY_FLOAT_INFINITY END AS "LogProba_0", 0.25 AS "Proba_1", CASE WHEN (0.25 IS NULL OR 0.25 > 0.0) THEN ln(0.25) ELSE -BINARY_FLOAT_INFINITY END AS "LogProba_1", 0.24 AS "Proba_2", CASE WHEN (0.24 IS NULL OR 0.24 > 0.0) THEN ln(0.24) ELSE -BINARY_FLOAT_INFINITY END AS "LogProba_2", 0.26 AS "Proba_3", CASE WHEN (0.26 IS NULL OR 0.26 > 0.0) THEN ln(0.26) ELSE -BINARY_FLOAT_INFINITY END AS "LogProba_3", multinomial_cte.multinomial_expr AS "Decision" 
