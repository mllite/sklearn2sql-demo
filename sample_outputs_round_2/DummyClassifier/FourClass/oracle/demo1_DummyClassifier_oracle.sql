...
FROM "INPUT_DATA" "ADS"), 
multinomial_cte AS 
(SELECT random_number_cte."KEY" AS "KEY", CASE WHEN (random_number_cte.uniform <= 0.25) THEN 0 ELSE CASE WHEN (random_number_cte.uniform <= 0.5) THEN 1 ELSE CASE WHEN (random_number_cte.uniform <= 0.75) THEN 2 ELSE CASE WHEN (random_number_cte.uniform <= 1.0) THEN 3 END END END END AS multinomial_expr 
