...
-- Code For temporary table TMP_20180508181012_CODEGEN_7NJNPN_ADS_imp_1_OUT part 1. Create 


CREATE TEMPORARY TABLE `TMP_20180508181012_CODEGEN_7NJNPN_ADS_imp_1_OUT` (
	`KEY` BIGINT NOT NULL, 
	impter_2 DOUBLE, 
	impter_3 DOUBLE, 
	impter_4 DOUBLE, 
...
-- Code For temporary table TMP_20180508181012_CODEGEN_7NJNPN_ADS_imp_1_OUT part 3. Create Index 

CREATE INDEX `ix_TMP_20180508181012_CODEGEN_7NJNPN_ADS_imp_1_OUT_KEY` ON `TMP_20180508181012_CODEGEN_7NJNPN_ADS_imp_1_OUT` (`KEY`)

-- Code For temporary table TMP_20180508181012_CODEGEN_UOA5A0_ADS_sca_2_OUT part 1. Create 


CREATE TEMPORARY TABLE `TMP_20180508181012_CODEGEN_UOA5A0_ADS_sca_2_OUT` (
	`KEY` BIGINT NOT NULL, 
	scaler_2 DOUBLE, 
	scaler_3 DOUBLE, 
	scaler_4 DOUBLE, 
...
FROM (SELECT `ADS_imp_1_OUT`.`KEY` AS `KEY`, (`ADS_imp_1_OUT`.impter_2 - 13.450740000000003) / 2.842552647252114 AS scaler_2, (`ADS_imp_1_OUT`.impter_3 - 19.4476) / 4.456612417520733 AS scaler_3, (`ADS_imp_1_OUT`.impter_4 - 87.29319999999998) / 19.783373366541916 AS scaler_4, (`ADS_imp_1_OUT`.impter_5 - 583.3380000000001) / 254.63059980293016 AS scaler_5, (`ADS_imp_1_OUT`.impter_6 - 0.09348100000000001) / 0.012981839045374118 AS scaler_6, (`ADS_imp_1_OUT`.impter_7 - 0.0959074) / 0.04794022510627167 AS scaler_7, (`ADS_imp_1_OUT`.impter_8 - 0.07945570000000003) / 0.072003293484465 AS scaler_8, (`ADS_imp_1_OUT`.impter_9 - 0.04142684) / 0.03286028997155077 AS scaler_9, (`ADS_imp_1_OUT`.impter_10 - 0.178872) / 0.023767574886807447 AS scaler_10, (`ADS_imp_1_OUT`.impter_11 - 0.0630418) / 0.006170491614126058 AS scaler_11, (`ADS_imp_1_OUT`.impter_12 - 0.3765220000000001) / 0.22931747451077505 AS scaler_12, (`ADS_imp_1_OUT`.impter_13 - 1.2955559999999997) / 0.4864979388075555 AS scaler_13, (`ADS_imp_1_OUT`.impter_14 - 2.7619079999999996) / 1.8449532326690556 AS scaler_14, (`ADS_imp_1_OUT`.impter_15 - 33.839960000000005) / 28.135330141983403 AS scaler_15, (`ADS_imp_1_OUT`.impter_16 - 0.006920639999999998) / 0.002156646106898394 AS scaler_16, (`ADS_imp_1_OUT`.impter_17 - 0.02546753999999999) / 0.019603893631837527 AS scaler_17, (`ADS_imp_1_OUT`.impter_18 - 0.033411) / 0.029953011615528748 AS scaler_18, (`ADS_imp_1_OUT`.impter_19 - 0.011515259999999998) / 0.006801917382062208 AS scaler_19, (`ADS_imp_1_OUT`.impter_20 - 0.02063639999999999) / 0.008194260127674737 AS scaler_20, (`ADS_imp_1_OUT`.impter_21 - 0.0038469800000000003) / 0.0021774517444940085 AS scaler_21, (`ADS_imp_1_OUT`.impter_22 - 15.393479999999997) / 3.899252873256619 AS scaler_22, (`ADS_imp_1_OUT`.impter_23 - 26.079600000000006) / 5.855341137798891 AS scaler_23, (`ADS_imp_1_OUT`.impter_24 - 101.605) / 28.256795094277766 AS scaler_24, (`ADS_imp_1_OUT`.impter_25 - 769.482) / 402.52477721998673 AS scaler_25, (`ADS_imp_1_OUT`.impter_26 - 0.12837400000000002) / 0.022491495459395314 AS scaler_26, (`ADS_imp_1_OUT`.impter_27 - 0.23153520000000005) / 0.13583353927863323 AS scaler_27, (`ADS_imp_1_OUT`.impter_28 - 0.2537764) / 0.19191074193759974 AS scaler_28, (`ADS_imp_1_OUT`.impter_29 - 0.10431359999999998) / 0.06250764832434509 AS scaler_29, (`ADS_imp_1_OUT`.impter_30 - 0.284744) / 0.05511358511292836 AS scaler_30, (`ADS_imp_1_OUT`.impter_31 - 0.08354739999999998) / 0.015270954103788016 AS scaler_31 
FROM `TMP_20180508181012_CODEGEN_7NJNPN_ADS_imp_1_OUT` AS `ADS_imp_1_OUT`) AS `ADS_sca_2_OUT`) AS `U`

-- Code For temporary table TMP_20180508181012_CODEGEN_UOA5A0_ADS_sca_2_OUT part 3. Create Index 

CREATE INDEX `ix_TMP_20180508181012_CODEGEN_UOA5A0_ADS_sca_2_OUT_KEY` ON `TMP_20180508181012_CODEGEN_UOA5A0_ADS_sca_2_OUT` (`KEY`)
...
FROM `TMP_20180508181012_CODEGEN_UOA5A0_ADS_sca_2_OUT` AS `ADS_sca_2_OUT`), 
`SV_data` AS 
...
FROM (SELECT full_join_data_sv.`KEY` AS `KEY`, sum(full_join_data_sv.dot_prod1) + 0.6932483317395821 AS dot_product 
...
FROM kernel_input, `SV_data`) AS full_join_data_sv GROUP BY full_join_data_sv.`KEY`) AS t)
 SELECT kernel_dp.`KEY` AS `KEY`, -kernel_dp.dot_product AS `Score_0`, kernel_dp.dot_product AS `Score_1`, 1.0 - 1.0 / (1.0 + exp(least(greatest(-100.0, -(-(kernel_dp.dot_product * -1.641023410878645 + 0.18180078941284394))), 100.0))) AS `Proba_0`, 1.0 / (1.0 + exp(least(greatest(-100.0, -(-(kernel_dp.dot_product * -1.641023410878645 + 0.18180078941284394))), 100.0))) AS `Proba_1`, CASE WHEN (1.0 - 1.0 / (1.0 + exp(least(greatest(-100.0, -(-(kernel_dp.dot_product * -1.641023410878645 + 0.18180078941284394))), 100.0))) IS NULL OR 1.0 - 1.0 / (1.0 + exp(least(greatest(-100.0, -(-(kernel_dp.dot_product * -1.641023410878645 + 0.18180078941284394))), 100.0))) > 0.0) THEN ln(1.0 - 1.0 / (1.0 + exp(least(greatest(-100.0, -(-(kernel_dp.dot_product * -1.641023410878645 + 0.18180078941284394))), 100.0)))) ELSE -1.79769313486231e+308 END AS `LogProba_0`, CASE WHEN (1.0 / (1.0 + exp(least(greatest(-100.0, -(-(kernel_dp.dot_product * -1.641023410878645 + 0.18180078941284394))), 100.0))) IS NULL OR 1.0 / (1.0 + exp(least(greatest(-100.0, -(-(kernel_dp.dot_product * -1.641023410878645 + 0.18180078941284394))), 100.0))) > 0.0) THEN ln(1.0 / (1.0 + exp(least(greatest(-100.0, -(-(kernel_dp.dot_product * -1.641023410878645 + 0.18180078941284394))), 100.0)))) ELSE -1.79769313486231e+308 END AS `LogProba_1`, CASE WHEN (kernel_dp.dot_product > 0.0) THEN 1 ELSE 0 END AS `Decision`, greatest(1.0 - 1.0 / (1.0 + exp(least(greatest(-100.0, -(-(kernel_dp.dot_product * -1.641023410878645 + 0.18180078941284394))), 100.0))), 1.0 / (1.0 + exp(least(greatest(-100.0, -(-(kernel_dp.dot_product * -1.641023410878645 + 0.18180078941284394))), 100.0)))) AS `DecisionProba` 
FROM kernel_dp