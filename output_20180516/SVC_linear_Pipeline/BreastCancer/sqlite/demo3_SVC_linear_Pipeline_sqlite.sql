...
-- Code For temporary table TMP_20180516111959_CODEGEN_FYY2OA_ADS_imp_1_OUT part 1. Create 


CREATE TEMPORARY TABLE "TMP_20180516111959_CODEGEN_FYY2OA_ADS_imp_1_OUT" (
	"KEY" BIGINT NOT NULL, 
	impter_2 FLOAT, 
	impter_3 FLOAT, 
	impter_4 FLOAT, 
...
-- Code For temporary table TMP_20180516111959_CODEGEN_FYY2OA_ADS_imp_1_OUT part 3. Create Index 

CREATE INDEX "ix_TMP_20180516111959_CODEGEN_FYY2OA_ADS_imp_1_OUT_KEY" ON "TMP_20180516111959_CODEGEN_FYY2OA_ADS_imp_1_OUT" ("KEY")

-- Code For temporary table TMP_20180516111959_CODEGEN_W1VE9F_ADS_sca_2_OUT part 1. Create 


CREATE TEMPORARY TABLE "TMP_20180516111959_CODEGEN_W1VE9F_ADS_sca_2_OUT" (
	"KEY" BIGINT NOT NULL, 
	scaler_2 FLOAT, 
	scaler_3 FLOAT, 
	scaler_4 FLOAT, 
...
FROM "TMP_20180516111959_CODEGEN_FYY2OA_ADS_imp_1_OUT" AS "ADS_imp_1_OUT") AS "ADS_sca_2_OUT"

-- Code For temporary table TMP_20180516111959_CODEGEN_W1VE9F_ADS_sca_2_OUT part 3. Create Index 

CREATE INDEX "ix_TMP_20180516111959_CODEGEN_W1VE9F_ADS_sca_2_OUT_KEY" ON "TMP_20180516111959_CODEGEN_W1VE9F_ADS_sca_2_OUT" ("KEY")
...
FROM "TMP_20180516111959_CODEGEN_W1VE9F_ADS_sca_2_OUT" AS "ADS_sca_2_OUT"), 
"SV_data" AS 
...
FROM kernel_input, "SV_data") AS full_join_data_sv GROUP BY full_join_data_sv."KEY") AS t)
 SELECT kernel_dp."KEY" AS "KEY", -kernel_dp.dot_product AS "Score_0", kernel_dp.dot_product AS "Score_1", 1.0 - 1.0 / (1.0 + exp(min(max(-100.0, -(-(kernel_dp.dot_product * -1.627929785039723 + 0.3475603146641396))), 100.0))) AS "Proba_0", 1.0 / (1.0 + exp(min(max(-100.0, -(-(kernel_dp.dot_product * -1.627929785039723 + 0.3475603146641396))), 100.0))) AS "Proba_1", CASE WHEN (1.0 - 1.0 / (1.0 + exp(min(max(-100.0, -(-(kernel_dp.dot_product * -1.627929785039723 + 0.3475603146641396))), 100.0))) IS NULL OR 1.0 - 1.0 / (1.0 + exp(min(max(-100.0, -(-(kernel_dp.dot_product * -1.627929785039723 + 0.3475603146641396))), 100.0))) > 0.0) THEN ln(1.0 - 1.0 / (1.0 + exp(min(max(-100.0, -(-(kernel_dp.dot_product * -1.627929785039723 + 0.3475603146641396))), 100.0)))) ELSE -1.79769313486231e+308 END AS "LogProba_0", CASE WHEN (1.0 / (1.0 + exp(min(max(-100.0, -(-(kernel_dp.dot_product * -1.627929785039723 + 0.3475603146641396))), 100.0))) IS NULL OR 1.0 / (1.0 + exp(min(max(-100.0, -(-(kernel_dp.dot_product * -1.627929785039723 + 0.3475603146641396))), 100.0))) > 0.0) THEN ln(1.0 / (1.0 + exp(min(max(-100.0, -(-(kernel_dp.dot_product * -1.627929785039723 + 0.3475603146641396))), 100.0)))) ELSE -1.79769313486231e+308 END AS "LogProba_1", CASE WHEN (kernel_dp.dot_product > 0.0) THEN 1 ELSE 0 END AS "Decision", max(1.0 - 1.0 / (1.0 + exp(min(max(-100.0, -(-(kernel_dp.dot_product * -1.627929785039723 + 0.3475603146641396))), 100.0))), 1.0 / (1.0 + exp(min(max(-100.0, -(-(kernel_dp.dot_product * -1.627929785039723 + 0.3475603146641396))), 100.0)))) AS "DecisionProba" 
FROM kernel_dp