...
-- Code For temporary table tmp_20180516114036_codegen_dltvva_quantiles part 1. Create 


CREATE TABLE tmp_20180516114036_codegen_dltvva_quantiles (
	`KEY` BIGINT, 
	`Quantile` DOUBLE
)
...
-- Code For temporary table tmp_20180516114036_codegen_dltvva_quantiles part 2. Populate
...
WHERE u1.`Estimator` >= u2.`Estimator` AND u1.`KEY` = u2.`KEY` GROUP BY u1.`KEY`, u1.est_index, u1.`Estimator`, u1.`Weight`) AS `CW`)
 INSERT INTO tmp_20180516114036_codegen_dltvva_quantiles (`KEY`, `Quantile`) SELECT `Quantiles`.`KEY`, `Quantiles`.`Quantile` 
...
FROM tmp_20180516114036_codegen_dltvva_quantiles AS `Quantiles`