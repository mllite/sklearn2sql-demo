...
-- Code For temporary table tmp_20180516131400_codegen_w0gc3t_ads_imp_1_out part 1. Create 


CREATE GLOBAL TEMPORARY TABLE tmp_20180516131400_codegen_w0gc3t_ads_imp_1_out  (
	"KEY" BIGINT, 
...
-- Code For temporary table tmp_20180516131400_codegen_w0gc3t_ads_imp_1_out part 2. Populate

INSERT INTO tmp_20180516131400_codegen_w0gc3t_ads_imp_1_out ("KEY", imputer_output_2, imputer_output_3, imputer_output_4, imputer_output_5) SELECT "U"."KEY", "U".imputer_output_2, "U".imputer_output_3, "U".imputer_output_4, "U".imputer_output_5 
...
-- Code For temporary table tmp_20180516131400_codegen_cx1yn3_ads_sca_2_out part 1. Create 


CREATE GLOBAL TEMPORARY TABLE tmp_20180516131400_codegen_cx1yn3_ads_sca_2_out  (
	"KEY" BIGINT, 
...
-- Code For temporary table tmp_20180516131400_codegen_cx1yn3_ads_sca_2_out part 2. Populate

INSERT INTO tmp_20180516131400_codegen_cx1yn3_ads_sca_2_out ("KEY", scaler_output_2, scaler_output_3, scaler_output_4, scaler_output_5) SELECT "U"."KEY", "U".scaler_output_2, "U".scaler_output_3, "U".scaler_output_4, "U".scaler_output_5 
...
FROM tmp_20180516131400_codegen_w0gc3t_ads_imp_1_out AS "ADS_imp_1_OUT") AS "ADS_sca_2_OUT") AS "U"
...
FROM tmp_20180516131400_codegen_cx1yn3_ads_sca_2_out AS "ADS_sca_2_OUT"), 
linear_model_cte AS 
(SELECT linear_input."KEY" AS "KEY", CAST(-6.446438699640585 AS DOUBLE PRECISION) * linear_input.scaler_output_2 + CAST(9.126807672452953 AS DOUBLE PRECISION) * linear_input.scaler_output_3 + CAST(-11.174112993481211 AS DOUBLE PRECISION) * linear_input.scaler_output_4 + CAST(-10.466657239707258 AS DOUBLE PRECISION) * linear_input.scaler_output_5 + CAST(-9.900812765034766 AS DOUBLE PRECISION) AS "Score_CLASS_0", CAST(-1.512360985252981 AS DOUBLE PRECISION) * linear_input.scaler_output_2 + CAST(-5.73412000363537 AS DOUBLE PRECISION) * linear_input.scaler_output_3 + CAST(5.486471191562016 AS DOUBLE PRECISION) * linear_input.scaler_output_4 + CAST(-15.011938670496466 AS DOUBLE PRECISION) * linear_input.scaler_output_5 + CAST(-4.785341385824414 AS DOUBLE PRECISION) AS "Score_CLASS_1", CAST(-6.881242482900888 AS DOUBLE PRECISION) * linear_input.scaler_output_2 + CAST(-12.232789341088875 AS DOUBLE PRECISION) * linear_input.scaler_output_3 + CAST(39.06367488392127 AS DOUBLE PRECISION) * linear_input.scaler_output_4 + CAST(29.85707802243188 AS DOUBLE PRECISION) * linear_input.scaler_output_5 + CAST(-36.8472415903434 AS DOUBLE PRECISION) AS "Score_CLASS_2" 
FROM linear_input), 
