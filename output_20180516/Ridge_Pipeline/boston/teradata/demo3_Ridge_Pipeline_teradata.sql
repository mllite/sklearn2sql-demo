...
-- Code For temporary table tmp_20180516114921_codegen_kj48jx_ads_imp_1_out part 1. Create 


CREATE GLOBAL TEMPORARY TABLE tmp_20180516114921_codegen_kj48jx_ads_imp_1_out  (
	"KEY" BIGINT, 
...
-- Code For temporary table tmp_20180516114921_codegen_kj48jx_ads_imp_1_out part 2. Populate

INSERT INTO tmp_20180516114921_codegen_kj48jx_ads_imp_1_out ("KEY", impter_2, impter_3, impter_4, impter_5, impter_6, impter_7, impter_8, impter_9, impter_10, impter_11, impter_12, impter_13, impter_14) SELECT "U"."KEY", "U".impter_2, "U".impter_3, "U".impter_4, "U".impter_5, "U".impter_6, "U".impter_7, "U".impter_8, "U".impter_9, "U".impter_10, "U".impter_11, "U".impter_12, "U".impter_13, "U".impter_14 
...
-- Code For temporary table tmp_20180516114921_codegen_5mrg4f_ads_sca_2_out part 1. Create 


CREATE GLOBAL TEMPORARY TABLE tmp_20180516114921_codegen_5mrg4f_ads_sca_2_out  (
	"KEY" BIGINT, 
...
FROM tmp_20180516114921_codegen_kj48jx_ads_imp_1_out AS "ADS_imp_1_OUT") AS "ADS_sca_2_OUT") AS "U"
...
FROM tmp_20180516114921_codegen_5mrg4f_ads_sca_2_out AS "ADS_sca_2_OUT"), 
linear_model_cte AS 
(SELECT linear_input."KEY" AS "KEY", CAST(-0.766041913792394 AS DOUBLE PRECISION) * linear_input.scaler_2 + CAST(0.883464727756144 AS DOUBLE PRECISION) * linear_input.scaler_3 + CAST(0.132375991410583 AS DOUBLE PRECISION) * linear_input.scaler_4 + CAST(0.628995263531277 AS DOUBLE PRECISION) * linear_input.scaler_5 + CAST(-1.71855927400647 AS DOUBLE PRECISION) * linear_input.scaler_6 + CAST(3.259869980609026 AS DOUBLE PRECISION) * linear_input.scaler_7 + CAST(-0.561763096742438 AS DOUBLE PRECISION) * linear_input.scaler_8 + CAST(-2.858886511587212 AS DOUBLE PRECISION) * linear_input.scaler_9 + CAST(2.268647862848082 AS DOUBLE PRECISION) * linear_input.scaler_10 + CAST(-1.96707783758745 AS DOUBLE PRECISION) * linear_input.scaler_11 + CAST(-2.154340153844844 AS DOUBLE PRECISION) * linear_input.scaler_12 + CAST(0.940261299902417 AS DOUBLE PRECISION) * linear_input.scaler_13 + CAST(-2.868751048435574 AS DOUBLE PRECISION) * linear_input.scaler_14 + CAST(22.578960396039605 AS DOUBLE PRECISION) AS "Estimator" 
FROM linear_input)
