...
-- Code For temporary table TMP_20180508185135_CODEGEN_0CPTXQ_ADS_imp_1_OUT part 1. Create 


CREATE TEMPORARY TABLE "TMP_20180508185135_CODEGEN_0CPTXQ_ADS_imp_1_OUT" (
	"KEY" BIGINT NOT NULL, 
...
-- Code For temporary table TMP_20180508185135_CODEGEN_0CPTXQ_ADS_imp_1_OUT part 2. Populate

INSERT INTO "TMP_20180508185135_CODEGEN_0CPTXQ_ADS_imp_1_OUT" ("KEY", imputer_output_2, imputer_output_3, imputer_output_4, imputer_output_5) SELECT "ADS_imp_1_OUT"."KEY", "ADS_imp_1_OUT".imputer_output_2, "ADS_imp_1_OUT".imputer_output_3, "ADS_imp_1_OUT".imputer_output_4, "ADS_imp_1_OUT".imputer_output_5 
...
-- Code For temporary table TMP_20180508185135_CODEGEN_0CPTXQ_ADS_imp_1_OUT part 3. Create Index 

CREATE INDEX "ix_TMP_20180508185135_CODEGEN_0CPTXQ_ADS_imp_1_OUT_KEY" ON "TMP_20180508185135_CODEGEN_0CPTXQ_ADS_imp_1_OUT" ("KEY")

-- Code For temporary table TMP_20180508185135_CODEGEN_OC33MM_ADS_sca_2_OUT part 1. Create 


CREATE TEMPORARY TABLE "TMP_20180508185135_CODEGEN_OC33MM_ADS_sca_2_OUT" (
	"KEY" BIGINT NOT NULL, 
...
-- Code For temporary table TMP_20180508185135_CODEGEN_OC33MM_ADS_sca_2_OUT part 2. Populate

INSERT INTO "TMP_20180508185135_CODEGEN_OC33MM_ADS_sca_2_OUT" ("KEY", scaler_output_2, scaler_output_3, scaler_output_4, scaler_output_5) SELECT "ADS_sca_2_OUT"."KEY", "ADS_sca_2_OUT".scaler_output_2, "ADS_sca_2_OUT".scaler_output_3, "ADS_sca_2_OUT".scaler_output_4, "ADS_sca_2_OUT".scaler_output_5 
...
FROM "TMP_20180508185135_CODEGEN_0CPTXQ_ADS_imp_1_OUT" AS "ADS_imp_1_OUT") AS "ADS_sca_2_OUT"

-- Code For temporary table TMP_20180508185135_CODEGEN_OC33MM_ADS_sca_2_OUT part 3. Create Index 

CREATE INDEX "ix_TMP_20180508185135_CODEGEN_OC33MM_ADS_sca_2_OUT_KEY" ON "TMP_20180508185135_CODEGEN_OC33MM_ADS_sca_2_OUT" ("KEY")
...
FROM "TMP_20180508185135_CODEGEN_OC33MM_ADS_sca_2_OUT" AS "ADS_sca_2_OUT"), 
"DT_node_data" AS 
(SELECT "Values".nid AS nid, CAST("Values"."E" AS FLOAT) AS "E" 
FROM (SELECT 3 AS nid, 0.6100168381304848 AS "E" UNION ALL SELECT 5 AS nid, 1.1409162524546155 AS "E" UNION ALL SELECT 8 AS nid, 1.350090108150775 AS "E" UNION ALL SELECT 9 AS nid, 1.3687175770364277 AS "E" UNION ALL SELECT 10 AS nid, 1.4092998433520778 AS "E" UNION ALL SELECT 13 AS nid, 0.8079109230317987 AS "E" UNION ALL SELECT 14 AS nid, 0.7050714675464733 AS "E" UNION ALL SELECT 16 AS nid, 0.4848669214064692 AS "E" UNION ALL SELECT 18 AS nid, 0.3751675659770648 AS "E" UNION ALL SELECT 19 AS nid, 0.38342473185592096 AS "E" UNION ALL SELECT 24 AS nid, 1.1156147639563705 AS "E" UNION ALL SELECT 25 AS nid, 1.1610071359052208 AS "E" UNION ALL SELECT 27 AS nid, 1.5359670902044162 AS "E" UNION ALL SELECT 28 AS nid, 1.4518609404362353 AS "E" UNION ALL SELECT 31 AS nid, 1.314325621172093 AS "E" UNION ALL SELECT 32 AS nid, 1.341344197483337 AS "E" UNION ALL SELECT 39 AS nid, 1.5696790399866454 AS "E" UNION ALL SELECT 40 AS nid, 1.570583991203967 AS "E" UNION ALL SELECT 41 AS nid, 1.5684866939082733 AS "E" UNION ALL SELECT 42 AS nid, 1.5644407243483047 AS "E" UNION ALL SELECT 43 AS nid, 1.5543288470291443 AS "E" UNION ALL SELECT 46 AS nid, 1.5278745466047203 AS "E" UNION ALL SELECT 48 AS nid, 1.536711230505171 AS "E" UNION ALL SELECT 49 AS nid, 1.541589567106448 AS "E" UNION ALL SELECT 51 AS nid, 1.556101969927248 AS "E" UNION ALL SELECT 52 AS nid, 1.5623228105763816 AS "E" UNION ALL SELECT 56 AS nid, 1.3678941404936502 AS "E" UNION ALL SELECT 58 AS nid, 1.410743936534041 AS "E" UNION ALL SELECT 59 AS nid, 1.4251802174855326 AS "E" UNION ALL SELECT 61 AS nid, 1.3844945172080796 AS "E" UNION ALL SELECT 63 AS nid, 1.534059281886407 AS "E" UNION ALL SELECT 67 AS nid, 1.496561917506885 AS "E" UNION ALL SELECT 68 AS nid, 1.499027385598966 AS "E" UNION ALL SELECT 69 AS nid, 1.4872578515104244 AS "E" UNION ALL SELECT 72 AS nid, 1.467648677980467 AS "E" UNION ALL SELECT 74 AS nid, 1.4642487267988717 AS "E" UNION ALL SELECT 75 AS nid, 1.4608890093126605 AS "E" UNION ALL SELECT 76 AS nid, 1.4929217934494698 AS "E" UNION ALL SELECT 79 AS nid, 1.4771263961886638 AS "E" UNION ALL SELECT 80 AS nid, 1.477951453333812 AS "E" UNION ALL SELECT 84 AS nid, 1.50778906671016 AS "E" UNION ALL SELECT 88 AS nid, 1.527762553523396 AS "E" UNION ALL SELECT 89 AS nid, 1.5318155845359613 AS "E" UNION ALL SELECT 91 AS nid, 1.5352537026948319 AS "E" UNION ALL SELECT 92 AS nid, 1.5325692862833142 AS "E" UNION ALL SELECT 94 AS nid, 1.5309500463495263 AS "E" UNION ALL SELECT 96 AS nid, 1.5218889384499552 AS "E" UNION ALL SELECT 97 AS nid, 1.521297715153797 AS "E" UNION ALL SELECT 99 AS nid, 1.5248539791622286 AS "E" UNION ALL SELECT 102 AS nid, 1.54561918356505 AS "E" UNION ALL SELECT 103 AS nid, 1.5438983791624072 AS "E" UNION ALL SELECT 104 AS nid, 1.5374002322115805 AS "E" UNION ALL SELECT 106 AS nid, 1.484710993001392 AS "E" UNION ALL SELECT 108 AS nid, 1.5032073896269296 AS "E" UNION ALL SELECT 109 AS nid, 1.5126619620701551 AS "E" UNION ALL SELECT 113 AS nid, 1.0274843519223331 AS "E" UNION ALL SELECT 114 AS nid, 1.076547084481779 AS "E" UNION ALL SELECT 118 AS nid, 1.1577860434316234 AS "E" UNION ALL SELECT 120 AS nid, 1.1082904037450438 AS "E" UNION ALL SELECT 121 AS nid, 1.1063441203172228 AS "E" UNION ALL SELECT 122 AS nid, 1.1968265037681187 AS "E" UNION ALL SELECT 124 AS nid, 1.1440564349145939 AS "E" UNION ALL SELECT 126 AS nid, 1.2791824090500283 AS "E" UNION ALL SELECT 127 AS nid, 1.26722341655496 AS "E" UNION ALL SELECT 129 AS nid, 1.0818920012987259 AS "E" UNION ALL SELECT 132 AS nid, 1.3498429932286293 AS "E" UNION ALL SELECT 133 AS nid, 1.3463614795691619 AS "E" UNION ALL SELECT 137 AS nid, 1.4719390289797578 AS "E" UNION ALL SELECT 138 AS nid, 1.4670826204003844 AS "E" UNION ALL SELECT 141 AS nid, 1.4860572646626233 AS "E" UNION ALL SELECT 142 AS nid, 1.4783281269798374 AS "E" UNION ALL SELECT 143 AS nid, 1.4949662645575934 AS "E" UNION ALL SELECT 145 AS nid, 1.4932233184960249 AS "E" UNION ALL SELECT 147 AS nid, 1.5028235051542993 AS "E" UNION ALL SELECT 148 AS nid, 1.5099001489420028 AS "E") AS "Values"), 
"DT_Output" AS 
