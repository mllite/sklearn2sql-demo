...
-- Code For temporary table tmp_20180509004143_codegen_5aua4o_lgbm_b0 part 1. Create 

CREATE TEMPORARY TABLE `tmp_20180509004143_codegen_5aua4o_lgbm_b0` STORED AS ORC AS WITH `DT_node_lookup` AS 
...
-- Code For temporary table tmp_20180509004143_codegen_5aua4o_lgbm_b0 part 2. Populate

SELECT * FROM `tmp_20180509004143_codegen_5aua4o_lgbm_b0`

-- Code For temporary table tmp_20180509004143_codegen_yeuuvs_lgbm_b1 part 1. Create 

CREATE TEMPORARY TABLE `tmp_20180509004143_codegen_yeuuvs_lgbm_b1` STORED AS ORC AS WITH `DT_node_lookup_10` AS 
(SELECT `ADS`.`KEY` AS `KEY`, CASE WHEN (`ADS`.`Feature_2` <= 0.5668879806967247) THEN 2 ELSE CASE WHEN (`ADS`.`Feature_1` <= 989.364790234492) THEN 6 ELSE 7 END END AS `node_id_2` 
FROM `freidman2` AS `ADS`), 
`DT_node_data_10` AS 
(SELECT `Values`.`nid` AS `nid`, CAST(`Values`.`Estimator` AS DOUBLE) AS `Estimator` 
...
-- Code For temporary table tmp_20180509004143_codegen_yeuuvs_lgbm_b1 part 2. Populate

SELECT * FROM `tmp_20180509004143_codegen_yeuuvs_lgbm_b1`

-- Code For temporary table tmp_20180509004143_codegen_slkgpk_lgbm_union part 1. Create 

CREATE TEMPORARY TABLE `tmp_20180509004143_codegen_slkgpk_lgbm_union` STORED AS ORC AS SELECT `LGBM_Union`.`KEY`, `LGBM_Union`.`Estimator` 
...
FROM `tmp_20180509004143_codegen_5aua4o_lgbm_b0` AS `LGBM_B0` UNION ALL SELECT `LGBM_B1`.`KEY` AS `KEY`, `LGBM_B1`.`Estimator` AS `Estimator` 
FROM `tmp_20180509004143_codegen_yeuuvs_lgbm_b1` AS `LGBM_B1`) AS `LGBM_EnsembleUnion`) AS `LGBM_Union`

-- Code For temporary table tmp_20180509004143_codegen_slkgpk_lgbm_union part 2. Populate

SELECT * FROM `tmp_20180509004143_codegen_slkgpk_lgbm_union`

-- Code For temporary table tmp_20180509004143_codegen_w441zu_lgbm_sum part 1. Create 

CREATE TEMPORARY TABLE `tmp_20180509004143_codegen_w441zu_lgbm_sum` STORED AS ORC AS SELECT `LGBM_sum`.`KEY`, `LGBM_sum`.`Estimator` 
...
FROM `tmp_20180509004143_codegen_slkgpk_lgbm_union` AS `LGBM_Union` GROUP BY `LGBM_Union`.`KEY`) AS `T`) AS `LGBM_sum`

-- Code For temporary table tmp_20180509004143_codegen_w441zu_lgbm_sum part 2. Populate

SELECT * FROM `tmp_20180509004143_codegen_w441zu_lgbm_sum`

-- Model deployment code

SELECT `LGBM_sum`.`KEY` AS `KEY`, `LGBM_sum`.`Estimator` AS `Estimator` 
FROM `tmp_20180509004143_codegen_w441zu_lgbm_sum` AS `LGBM_sum`