...
-- Code For temporary table tmp_20180516113712_codegen_u0pc44_lgbm_b0 part 1. Create 

CREATE TEMPORARY TABLE `tmp_20180516113712_codegen_u0pc44_lgbm_b0` STORED AS ORC AS WITH `DT_node_lookup` AS 
...
-- Code For temporary table tmp_20180516113712_codegen_u0pc44_lgbm_b0 part 2. Populate

SELECT * FROM `tmp_20180516113712_codegen_u0pc44_lgbm_b0`

-- Code For temporary table tmp_20180516113712_codegen_4cnr5h_lgbm_b1 part 1. Create 

CREATE TEMPORARY TABLE `tmp_20180516113712_codegen_4cnr5h_lgbm_b1` STORED AS ORC AS WITH `DT_node_lookup_10` AS 
(SELECT `ADS`.`KEY` AS `KEY`, CASE WHEN (`ADS`.`Feature_1` <= -0.19500725638808467) THEN 2 ELSE CASE WHEN (`ADS`.`Feature_8` <= -0.08420545265254369) THEN 6 ELSE 7 END END AS `node_id_2` 
FROM `RandomReg_10` AS `ADS`), 
`DT_node_data_10` AS 
(SELECT `Values`.`nid` AS `nid`, CAST(`Values`.`Estimator` AS DOUBLE) AS `Estimator` 
...
-- Code For temporary table tmp_20180516113712_codegen_4cnr5h_lgbm_b1 part 2. Populate

SELECT * FROM `tmp_20180516113712_codegen_4cnr5h_lgbm_b1`

-- Code For temporary table tmp_20180516113712_codegen_8p9aok_lgbm_sum part 1. Create 

CREATE TEMPORARY TABLE `tmp_20180516113712_codegen_8p9aok_lgbm_sum` STORED AS ORC AS WITH `LGBM_Union` AS 
...
FROM `tmp_20180516113712_codegen_u0pc44_lgbm_b0` AS `LGBM_B0` UNION ALL SELECT `LGBM_B1`.`KEY` AS `KEY`, `LGBM_B1`.`Estimator` AS `Estimator` 
FROM `tmp_20180516113712_codegen_4cnr5h_lgbm_b1` AS `LGBM_B1`) AS `LGBM_EnsembleUnion`)
...
-- Code For temporary table tmp_20180516113712_codegen_8p9aok_lgbm_sum part 2. Populate

SELECT * FROM `tmp_20180516113712_codegen_8p9aok_lgbm_sum`

-- Model deployment code

SELECT `LGBM_sum`.`KEY` AS `KEY`, `LGBM_sum`.`Estimator` AS `Estimator` 
FROM `tmp_20180516113712_codegen_8p9aok_lgbm_sum` AS `LGBM_sum`