...
-- Code For temporary table tmp_20180516121611_codegen_anmna3_xgb_b0 part 1. Create 

CREATE TEMPORARY TABLE `tmp_20180516121611_codegen_anmna3_xgb_b0` STORED AS ORC AS WITH `DT_node_lookup` AS 
...
-- Code For temporary table tmp_20180516121611_codegen_anmna3_xgb_b0 part 2. Populate

SELECT * FROM `tmp_20180516121611_codegen_anmna3_xgb_b0`

-- Code For temporary table tmp_20180516121611_codegen_gaqegx_xgb_b1 part 1. Create 

CREATE TEMPORARY TABLE `tmp_20180516121611_codegen_gaqegx_xgb_b1` STORED AS ORC AS WITH `DT_node_lookup_10` AS 
(SELECT `ADS`.`KEY` AS `KEY`, CASE WHEN (`ADS`.`Feature_2` < 0.792053759098053) THEN CASE WHEN (`ADS`.`Feature_1` < 784.8369140625) THEN CASE WHEN (`ADS`.`Feature_1` < 476.31121826171875) THEN 5 ELSE 6 END ELSE CASE WHEN (`ADS`.`Feature_2` < 0.288340300321579) THEN 7 ELSE 8 END END ELSE 2 END AS `node_id_2` 
FROM `freidman2` AS `ADS`), 
`DT_node_data_10` AS 
(SELECT `Values`.`nid` AS `nid`, CAST(`Values`.`Estimator` AS DOUBLE) AS `Estimator` 
...
-- Code For temporary table tmp_20180516121611_codegen_gaqegx_xgb_b1 part 2. Populate

SELECT * FROM `tmp_20180516121611_codegen_gaqegx_xgb_b1`

-- Code For temporary table tmp_20180516121611_codegen_pwe4ar_xgb_sum part 1. Create 

CREATE TEMPORARY TABLE `tmp_20180516121611_codegen_pwe4ar_xgb_sum` STORED AS ORC AS WITH `XGB_Union` AS 
...
FROM `tmp_20180516121611_codegen_anmna3_xgb_b0` AS `XGB_B0` UNION ALL SELECT `XGB_B1`.`KEY` AS `KEY`, `XGB_B1`.`Estimator` AS `Estimator` 
FROM `tmp_20180516121611_codegen_gaqegx_xgb_b1` AS `XGB_B1`) AS `XGB_EnsembleUnion`)
...
-- Code For temporary table tmp_20180516121611_codegen_pwe4ar_xgb_sum part 2. Populate

SELECT * FROM `tmp_20180516121611_codegen_pwe4ar_xgb_sum`

-- Model deployment code

SELECT `XGB_sum`.`KEY` AS `KEY`, `XGB_sum`.`Estimator` + 0.5 AS `Estimator` 
FROM `tmp_20180516121611_codegen_pwe4ar_xgb_sum` AS `XGB_sum`