...
-- Code For temporary table tmp_20180417051050_ydnmft_ads_ part 1/2. Create 


CREATE GLOBAL TEMPORARY TABLE tmp_20180417051050_ydnmft_ads_  (
	"KEY" BIGINT, 
...
-- Code For temporary table tmp_20180417051050_ydnmft_ads_ part 2/2. Populate

INSERT INTO tmp_20180417051050_ydnmft_ads_ ("KEY", imputer_output_2, imputer_output_3, imputer_output_4, imputer_output_5) SELECT "U"."KEY", "U".imputer_output_2, "U".imputer_output_3, "U".imputer_output_4, "U".imputer_output_5 
...
-- Code For temporary table tmp_20180417051050_iuap1j_ads_ part 1/2. Create 


CREATE GLOBAL TEMPORARY TABLE tmp_20180417051050_iuap1j_ads_  (
	"KEY" BIGINT, 
...
-- Code For temporary table tmp_20180417051050_iuap1j_ads_ part 2/2. Populate

INSERT INTO tmp_20180417051050_iuap1j_ads_ ("KEY", scaler_output_2, scaler_output_3, scaler_output_4, scaler_output_5) SELECT "U"."KEY", "U".scaler_output_2, "U".scaler_output_3, "U".scaler_output_4, "U".scaler_output_5 
...
FROM tmp_20180417051050_ydnmft_ads_ AS "ADS_imp_1_OUT") AS "ADS_sca_2_OUT") AS "U"
...
FROM tmp_20180417051050_iuap1j_ads_ AS "ADS_sca_2_OUT"), 
linear_model_cte AS 
(SELECT linear_input."KEY" AS "KEY", -0.76677823912151 * linear_input.scaler_output_2 + 1.344886168177664 * linear_input.scaler_output_3 + -1.580922603723415 * linear_input.scaler_output_4 + -1.355847945669726 * linear_input.scaler_output_5 + -1.682812270560116 AS "Score_CLASS_0", 0.25942149562529 * linear_input.scaler_output_2 + -1.18169600578164 * linear_input.scaler_output_3 + 0.591551464722226 * linear_input.scaler_output_4 + -0.865308515158648 * linear_input.scaler_output_5 + -0.857494023365802 AS "Score_CLASS_1", -0.0016242729027 * linear_input.scaler_output_2 + -0.208887231569124 * linear_input.scaler_output_3 + 1.877417535343714 * linear_input.scaler_output_4 + 2.521926697400801 * linear_input.scaler_output_5 + -2.350481003984672 AS "Score_CLASS_2" 
FROM linear_input), 
orig_cte AS 
(SELECT linear_model_cte."KEY" AS "KEY", linear_model_cte."Score_CLASS_0" AS "Score_CLASS_0", linear_model_cte."Score_CLASS_1" AS "Score_CLASS_1", linear_model_cte."Score_CLASS_2" AS "Score_CLASS_2", (1.0 / (1.0 + exp(least(greatest(-100.0, -linear_model_cte."Score_CLASS_0"), 100.0)))) / (1.0 / (1.0 + exp(least(greatest(-100.0, -linear_model_cte."Score_CLASS_0"), 100.0))) + 1.0 / (1.0 + exp(least(greatest(-100.0, -linear_model_cte."Score_CLASS_1"), 100.0))) + 1.0 / (1.0 + exp(least(greatest(-100.0, -linear_model_cte."Score_CLASS_2"), 100.0)))) AS "Proba_CLASS_0", (1.0 / (1.0 + exp(least(greatest(-100.0, -linear_model_cte."Score_CLASS_1"), 100.0)))) / (1.0 / (1.0 + exp(least(greatest(-100.0, -linear_model_cte."Score_CLASS_0"), 100.0))) + 1.0 / (1.0 + exp(least(greatest(-100.0, -linear_model_cte."Score_CLASS_1"), 100.0))) + 1.0 / (1.0 + exp(least(greatest(-100.0, -linear_model_cte."Score_CLASS_2"), 100.0)))) AS "Proba_CLASS_1", (1.0 / (1.0 + exp(least(greatest(-100.0, -linear_model_cte."Score_CLASS_2"), 100.0)))) / (1.0 / (1.0 + exp(least(greatest(-100.0, -linear_model_cte."Score_CLASS_0"), 100.0))) + 1.0 / (1.0 + exp(least(greatest(-100.0, -linear_model_cte."Score_CLASS_1"), 100.0))) + 1.0 / (1.0 + exp(least(greatest(-100.0, -linear_model_cte."Score_CLASS_2"), 100.0)))) AS "Proba_CLASS_2", CAST(NULL AS FLOAT) AS "LogProba_CLASS_0", CAST(NULL AS FLOAT) AS "LogProba_CLASS_1", CAST(NULL AS FLOAT) AS "LogProba_CLASS_2", CAST(NULL AS BIGINT) AS "Decision", CAST(NULL AS FLOAT) AS "DecisionProba" 
