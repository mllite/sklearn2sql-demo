...
-- Code For temporary table TMP_20180417051049_Y0F2TQ_ADS_imp_1_OUT part 1/2. Create 


CREATE TEMPORARY TABLE "TMP_20180417051049_Y0F2TQ_ADS_imp_1_OUT" (
	"KEY" BIGINT NOT NULL, 
...
-- Code For temporary table TMP_20180417051049_Y0F2TQ_ADS_imp_1_OUT part 2/2. Populate

INSERT INTO "TMP_20180417051049_Y0F2TQ_ADS_imp_1_OUT" ("KEY", imputer_output_2, imputer_output_3, imputer_output_4, imputer_output_5) SELECT "ADS_imp_1_OUT"."KEY", "ADS_imp_1_OUT".imputer_output_2, "ADS_imp_1_OUT".imputer_output_3, "ADS_imp_1_OUT".imputer_output_4, "ADS_imp_1_OUT".imputer_output_5 
...
-- Code For temporary table TMP_20180417051049_FUN6O3_ADS_sca_2_OUT part 1/2. Create 


CREATE TEMPORARY TABLE "TMP_20180417051049_FUN6O3_ADS_sca_2_OUT" (
	"KEY" BIGINT NOT NULL, 
...
-- Code For temporary table TMP_20180417051049_FUN6O3_ADS_sca_2_OUT part 2/2. Populate

INSERT INTO "TMP_20180417051049_FUN6O3_ADS_sca_2_OUT" ("KEY", scaler_output_2, scaler_output_3, scaler_output_4, scaler_output_5) SELECT "ADS_sca_2_OUT"."KEY", "ADS_sca_2_OUT".scaler_output_2, "ADS_sca_2_OUT".scaler_output_3, "ADS_sca_2_OUT".scaler_output_4, "ADS_sca_2_OUT".scaler_output_5 
...
FROM "TMP_20180417051049_Y0F2TQ_ADS_imp_1_OUT" AS "ADS_imp_1_OUT") AS "ADS_sca_2_OUT"
...
FROM "TMP_20180417051049_FUN6O3_ADS_sca_2_OUT" AS "ADS_sca_2_OUT"), 
linear_model_cte AS 
...
FROM linear_input), 
orig_cte AS 
(SELECT linear_model_cte."KEY" AS "KEY", linear_model_cte."Score_CLASS_0" AS "Score_CLASS_0", linear_model_cte."Score_CLASS_1" AS "Score_CLASS_1", linear_model_cte."Score_CLASS_2" AS "Score_CLASS_2", (1.0 / (1.0 + exp(min(max(-100.0, -linear_model_cte."Score_CLASS_0"), 100.0)))) / (1.0 / (1.0 + exp(min(max(-100.0, -linear_model_cte."Score_CLASS_0"), 100.0))) + 1.0 / (1.0 + exp(min(max(-100.0, -linear_model_cte."Score_CLASS_1"), 100.0))) + 1.0 / (1.0 + exp(min(max(-100.0, -linear_model_cte."Score_CLASS_2"), 100.0)))) AS "Proba_CLASS_0", (1.0 / (1.0 + exp(min(max(-100.0, -linear_model_cte."Score_CLASS_1"), 100.0)))) / (1.0 / (1.0 + exp(min(max(-100.0, -linear_model_cte."Score_CLASS_0"), 100.0))) + 1.0 / (1.0 + exp(min(max(-100.0, -linear_model_cte."Score_CLASS_1"), 100.0))) + 1.0 / (1.0 + exp(min(max(-100.0, -linear_model_cte."Score_CLASS_2"), 100.0)))) AS "Proba_CLASS_1", (1.0 / (1.0 + exp(min(max(-100.0, -linear_model_cte."Score_CLASS_2"), 100.0)))) / (1.0 / (1.0 + exp(min(max(-100.0, -linear_model_cte."Score_CLASS_0"), 100.0))) + 1.0 / (1.0 + exp(min(max(-100.0, -linear_model_cte."Score_CLASS_1"), 100.0))) + 1.0 / (1.0 + exp(min(max(-100.0, -linear_model_cte."Score_CLASS_2"), 100.0)))) AS "Proba_CLASS_2", CAST(NULL AS FLOAT) AS "LogProba_CLASS_0", CAST(NULL AS FLOAT) AS "LogProba_CLASS_1", CAST(NULL AS FLOAT) AS "LogProba_CLASS_2", CAST(NULL AS BIGINT) AS "Decision", CAST(NULL AS FLOAT) AS "DecisionProba" 
