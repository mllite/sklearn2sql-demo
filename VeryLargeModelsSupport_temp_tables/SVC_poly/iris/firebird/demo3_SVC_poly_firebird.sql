...
-- Code For temporary table TMP_20180417042416_OG6I96_KERNA part 1/2. Create 


CREATE GLOBAL TEMPORARY TABLE "TMP_20180417042416_OG6I96_KERNA" (
	"KEY" BIGINT, 
	"OVO_Class1" BIGINT, 
	"OVO_Class2" BIGINT, 
	"OVO_Decision" BIGINT, 
...
-- Code For temporary table TMP_20180417042416_OG6I96_KERNA part 2/2. Populate

INSERT INTO "TMP_20180417042416_OG6I96_KERNA" ("KEY", "OVO_Class1", "OVO_Class2", "OVO_Decision", "OVO_Confidence") SELECT "U"."KEY", "U"."OVO_Class1", "U"."OVO_Class2", "U"."OVO_Decision", "U"."OVO_Confidence" 
...
kernel_cte_0_1 AS 
(SELECT kernel_dp_0_1."KEY" AS "KEY", 0 AS "OVO_Class1", 1 AS "OVO_Class2", CASE WHEN (-kernel_dp_0_1.dot_product >= 0.0) THEN 1 ELSE 0 END AS "OVO_Decision", coalesce(-kernel_dp_0_1.dot_product, 1.0 / (1.0 + exp(CASE WHEN (minvalue(maxvalue(-100.0, -(-((-kernel_dp_0_1.dot_product) * -1.685346006327004 + -0.8663537572971911))), 100.0) >= -709.782712893384) THEN minvalue(maxvalue(-100.0, -(-((-kernel_dp_0_1.dot_product) * -1.685346006327004 + -0.8663537572971911))), 100.0) ELSE -709.782712893384 END))) AS "OVO_Confidence" 
FROM kernel_dp_0_1), 
"SV_data_0_2" AS 
(SELECT "Values".sv_idx AS sv_idx, CAST("Values".dual_coeff AS FLOAT) AS dual_coeff, CAST("Values".sv_0 AS FLOAT) AS sv_0, CAST("Values".sv_1 AS FLOAT) AS sv_1, CAST("Values".sv_2 AS FLOAT) AS sv_2, CAST("Values".sv_3 AS FLOAT) AS sv_3 
FROM (SELECT 0 AS sv_idx, 0.0006286056310521933 AS dual_coeff, 4.8 AS sv_0, 3.4 AS sv_1, 1.9 AS sv_2, 0.2 AS sv_3 FROM rdb$database UNION ALL SELECT 1 AS sv_idx, 0.0005066779305344128 AS dual_coeff, 5.1 AS sv_0, 3.3 AS sv_1, 1.7 AS sv_2, 0.5 AS sv_3 FROM rdb$database UNION ALL SELECT 2 AS sv_idx, 0.0006814761767680465 AS dual_coeff, 5.1 AS sv_0, 3.8 AS sv_1, 1.9 AS sv_2, 0.4 AS sv_3 FROM rdb$database UNION ALL SELECT 3 AS sv_idx, 0.0 AS dual_coeff, 4.5 AS sv_0, 2.3 AS sv_1, 1.3 AS sv_2, 0.3 AS sv_3 FROM rdb$database UNION ALL SELECT 9 AS sv_idx, -0.0 AS dual_coeff, 6.1 AS sv_0, 3.0 AS sv_1, 4.9 AS sv_2, 1.8 AS sv_3 FROM rdb$database UNION ALL SELECT 10 AS sv_idx, -0.0018167597383546528 AS dual_coeff, 4.9 AS sv_0, 2.5 AS sv_1, 4.5 AS sv_2, 1.7 AS sv_3 FROM rdb$database UNION ALL SELECT 11 AS sv_idx, -0.0 AS dual_coeff, 6.3 AS sv_0, 2.7 AS sv_1, 4.9 AS sv_2, 1.8 AS sv_3 FROM rdb$database UNION ALL SELECT 12 AS sv_idx, -0.0 AS dual_coeff, 6.0 AS sv_0, 3.0 AS sv_1, 4.8 AS sv_2, 1.8 AS sv_3 FROM rdb$database UNION ALL SELECT 13 AS sv_idx, -0.0 AS dual_coeff, 6.3 AS sv_0, 2.8 AS sv_1, 5.1 AS sv_2, 1.5 AS sv_3 FROM rdb$database) AS "Values"), 
kernel_dp_0_2 AS 
(SELECT t_1."KEY" AS "KEY", t_1.dot_product AS dot_product 
FROM (SELECT full_join_data_sv_1."KEY" AS "KEY", sum(CAST(full_join_data_sv_1.dot_prod1 AS FLOAT)) + 1.1844509506518808 AS dot_product 
...
kernel_cte_0_2 AS 
(SELECT kernel_dp_0_2."KEY" AS "KEY", 0 AS "OVO_Class1", 2 AS "OVO_Class2", CASE WHEN (-kernel_dp_0_2.dot_product >= 0.0) THEN 1 ELSE 0 END AS "OVO_Decision", coalesce(-kernel_dp_0_2.dot_product, 1.0 / (1.0 + exp(CASE WHEN (minvalue(maxvalue(-100.0, -(-((-kernel_dp_0_2.dot_product) * -1.9686581541997699 + -0.8210785452893736))), 100.0) >= -709.782712893384) THEN minvalue(maxvalue(-100.0, -(-((-kernel_dp_0_2.dot_product) * -1.9686581541997699 + -0.8210785452893736))), 100.0) ELSE -709.782712893384 END))) AS "OVO_Confidence" 
FROM kernel_dp_0_2), 
"SV_data_1_2" AS 
(SELECT "Values".sv_idx AS sv_idx, CAST("Values".dual_coeff AS FLOAT) AS dual_coeff, CAST("Values".sv_0 AS FLOAT) AS sv_0, CAST("Values".sv_1 AS FLOAT) AS sv_1, CAST("Values".sv_2 AS FLOAT) AS sv_2, CAST("Values".sv_3 AS FLOAT) AS sv_3 
FROM (SELECT 4 AS sv_idx, 0.0 AS dual_coeff, 5.1 AS sv_0, 2.5 AS sv_1, 3.0 AS sv_2, 1.1 AS sv_3 FROM rdb$database UNION ALL SELECT 5 AS sv_idx, 0.1 AS dual_coeff, 5.9 AS sv_0, 3.2 AS sv_1, 4.8 AS sv_2, 1.8 AS sv_3 FROM rdb$database UNION ALL SELECT 6 AS sv_idx, 0.1 AS dual_coeff, 6.3 AS sv_0, 2.5 AS sv_1, 4.9 AS sv_2, 1.5 AS sv_3 FROM rdb$database UNION ALL SELECT 7 AS sv_idx, 0.037538393414461316 AS dual_coeff, 5.4 AS sv_0, 3.0 AS sv_1, 4.5 AS sv_2, 1.5 AS sv_3 FROM rdb$database UNION ALL SELECT 8 AS sv_idx, 0.1 AS dual_coeff, 6.7 AS sv_0, 3.0 AS sv_1, 5.0 AS sv_2, 1.7 AS sv_3 FROM rdb$database UNION ALL SELECT 9 AS sv_idx, -0.06751756546833149 AS dual_coeff, 6.1 AS sv_0, 3.0 AS sv_1, 4.9 AS sv_2, 1.8 AS sv_3 FROM rdb$database UNION ALL SELECT 10 AS sv_idx, -0.0 AS dual_coeff, 4.9 AS sv_0, 2.5 AS sv_1, 4.5 AS sv_2, 1.7 AS sv_3 FROM rdb$database UNION ALL SELECT 11 AS sv_idx, -0.07002082794612988 AS dual_coeff, 6.3 AS sv_0, 2.7 AS sv_1, 4.9 AS sv_2, 1.8 AS sv_3 FROM rdb$database UNION ALL SELECT 12 AS sv_idx, -0.1 AS dual_coeff, 6.0 AS sv_0, 3.0 AS sv_1, 4.8 AS sv_2, 1.8 AS sv_3 FROM rdb$database UNION ALL SELECT 13 AS sv_idx, -0.1 AS dual_coeff, 6.3 AS sv_0, 2.8 AS sv_1, 5.1 AS sv_2, 1.5 AS sv_3 FROM rdb$database) AS "Values"), 
kernel_dp_1_2 AS 
...
kernel_cte_1_2 AS 
(SELECT kernel_dp_1_2."KEY" AS "KEY", 1 AS "OVO_Class1", 2 AS "OVO_Class2", CASE WHEN (-kernel_dp_1_2.dot_product >= 0.0) THEN 1 ELSE 0 END AS "OVO_Decision", coalesce(-kernel_dp_1_2.dot_product, 1.0 / (1.0 + exp(CASE WHEN (minvalue(maxvalue(-100.0, -(-((-kernel_dp_1_2.dot_product) * -0.9124033225944035 + -0.11978768191027243))), 100.0) >= -709.782712893384) THEN minvalue(maxvalue(-100.0, -(-((-kernel_dp_1_2.dot_product) * -0.9124033225944035 + -0.11978768191027243))), 100.0) ELSE -709.782712893384 END))) AS "OVO_Confidence" 
FROM kernel_dp_1_2)
...
-- Code For temporary table TMP_20180417042416_FN7C2F_KERNA part 1/2. Create 


CREATE GLOBAL TEMPORARY TABLE "TMP_20180417042416_FN7C2F_KERNA" (
	"KEY" BIGINT, 
	"OVO_Class1" BIGINT, 
	"OVO_Class2" BIGINT, 
	"OVO_Decision" BIGINT, 
...
-- Code For temporary table TMP_20180417042416_FN7C2F_KERNA part 2/2. Populate

INSERT INTO "TMP_20180417042416_FN7C2F_KERNA" ("KEY", "OVO_Class1", "OVO_Class2", "OVO_Decision", "OVO_Confidence") SELECT "U"."KEY", "U"."OVO_Class1", "U"."OVO_Class2", "U"."OVO_Decision", "U"."OVO_Confidence" 
...
FROM "TMP_20180417042416_OG6I96_KERNA" AS "KernAgg_B0") AS "KernAgg_EnsembleUnion") AS "KernAgg_Union") AS "U"

-- Code For temporary table TMP_20180417042416_E51KEL_KERNA part 1/2. Create 


CREATE GLOBAL TEMPORARY TABLE "TMP_20180417042416_E51KEL_KERNA" (
	"KEY" BIGINT, 
	"OVO_Vote_0" BIGINT, 
...
-- Code For temporary table TMP_20180417042416_E51KEL_KERNA part 2/2. Populate

INSERT INTO "TMP_20180417042416_E51KEL_KERNA" ("KEY", "OVO_Vote_0", "OVO_SumConfidence_0", "OVO_Vote_1", "OVO_SumConfidence_1", "OVO_Vote_2", "OVO_SumConfidence_2", max_abs_confidence, "OVO_Scale", "Score_0", "Score_1", "Score_2") SELECT "U"."KEY", "U"."OVO_Vote_0", "U"."OVO_SumConfidence_0", "U"."OVO_Vote_1", "U"."OVO_SumConfidence_1", "U"."OVO_Vote_2", "U"."OVO_SumConfidence_2", "U".max_abs_confidence, "U"."OVO_Scale", "U"."Score_0", "U"."Score_1", "U"."Score_2" 
...
FROM "TMP_20180417042416_FN7C2F_KERNA" AS "KernAgg_Union" GROUP BY "KernAgg_Union"."KEY") AS "Votes_Sel"), 
"OVO_SumConf_CTE" AS 
...
FROM "TMP_20180417042416_E51KEL_KERNA" AS "KernAgg"), 
